...
4. Select the `platform-context-graph-0.1.0.vsix` file
...
1. **Check Extension is Active**
   ...
   - Search for "PlatformContextGraph"
   - Should show as installed

2. **Check Activity Bar Icon**
   - Look for the PCG icon in the left sidebar
   - Click it to open the PCG explorer

### Test 2: Index a Workspace

1. **Open a Test Project**
   ```bash
   # Use the PlatformContextGraph project itself as a test
...
3. **Verify Indexing**
   - Check the Projects panel in the PCG sidebar
...
1. **Enable Diagnostics** (if not already enabled)
   - Press `Cmd+,` (Mac) or `Ctrl+,` (Windows/Linux)
   - Search for "pcg.enableDiagnostics"
   - Ensure it's checked
...
1. **View Dependencies**
   ...
   - Check the "Dependencies" panel in the PCG sidebar
...
1. **Find Dead Code**
   - Press `Cmd+Shift+P` (Mac) or `Ctrl+Shift+P` (Windows/Linux)
   - Type "PCG: Find Dead Code"
   - Should show unused functions/classes

2. **Analyze Complexity**
   - Press `Cmd+Shift+P` (Mac) or `Ctrl+Shift+P` (Windows/Linux)
   - Type "PCG: Analyze Complexity"
   - Should show complex functions

3. **Show Statistics**
   - Press `Cmd+Shift+P` (Mac) or `Ctrl+Shift+P` (Windows/Linux)
   - Type "PCG: Show Statistics"
...
1. **Check Status Bar**
   ...
   - Should see "$(database) PCG: Ready"

2. **Click Status Bar**
   - Click on the PCG status bar item
...
**Solutions**:
1. Check if extension is installed: `code --list-extensions | grep platformcontext.platform-context-graph`
2. Reload VS Code window: `Cmd+Shift+P` → "Reload Window"
3. Check Output panel: View → Output → Select "PlatformContextGraph"

### "pcg: command not found"

**Symptoms**: Commands fail with "pcg not found"

**Solutions**:
1. Verify pcg is installed: `which pcg`
2. Install pcg: `pip install platform-context-graph`
3. Set custom path in settings: `pcg.cliPath`
...
**Solutions**:
1. Index the workspace: "PCG: Index Current Workspace"
2. Check pcg is working: `pcg list` in terminal
3. Re-index: "PCG: Re-index Current Workspace"
...
**Solutions**:
1. Enable in settings: `pcg.enableCodeLens`
...
**Solutions**:
1. Enable in settings: `pcg.enableDiagnostics`
2. Save the file to trigger diagnostics
3. Check complexity threshold: `pcg.complexityThreshold`
...
- [ ] PCG icon appears in activity bar
...
If you encounter any issues, please report them on [GitHub](https://github.com/platformcontext/platform-context-graph/issues).
