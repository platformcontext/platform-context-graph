...
import { PcgManager, FunctionInfo } from '../pcgManager';
...
export class CallGraphTreeProvider implements vscode.TreeDataProvider<CallGraphTreeItem> {
    ...
    async getChildren(element?: CallGraphTreeItem): Promise<CallGraphTreeItem[]> {
        if (!element) {
            ...
            // Show callers
            try {
                const callers = await this.pcgManager.getCallers(this.currentFunction!);
            ...
            // Show callees
            try {
                const callees = await this.pcgManager.getCallees(this.currentFunction!);
