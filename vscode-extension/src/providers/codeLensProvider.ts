...
export class PcgCodeLensProvider implements vscode.CodeLensProvider {
    ...
    public async provideCodeLenses(
        ...
        try {
            ...
            const functions = await this.pcgManager.getFunctions();
            ...
            for (const func of fileFunctions) {
                ...
                // Get callers count
                const callers = await this.pcgManager.getCallers(func.name);
                const callersCount = callers.length;

                // Get callees count
                const callees = await this.pcgManager.getCallees(func.name);
