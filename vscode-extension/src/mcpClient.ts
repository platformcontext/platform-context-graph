...
export class McpClient {
    ...
    constructor(pcgPath: string, workspaceRoot: string, outputChannel?: vscode.OutputChannel) {
        ...
        this.process.stderr?.on('data', (data) => {
            ...
            stderrBuf += msg;
            this.outputChannel?.appendLine(`[PCG Server] ${msg.trim()}`);
        });

        this.process.on('error', (error) => {
            const msg = `PCG Server Process Error: ${error.message}\n` +
                `Make sure the PCG path is correct in settings (pcg.cliPath).`;
        ...
        this.process.on('exit', (code) => {
            if (code !== 0 && code !== null) {
                const msg = `PCG Server exited with code ${code}`;
                this.outputChannel?.appendLine(msg);
                if (stderrBuf.toLowerCase().includes('connection refused') ||
                    stderrBuf.toLowerCase().includes('unable to connect')) {
                    vscode.window.showErrorMessage(
                        '❌ PCG: Database connection failed. ' +
                        'Open PCG Configuration (PCG: Open Configuration) to set your .env variables.',
                        'Open Configuration'
                    ).then(choice => {
                        if (choice === 'Open Configuration') {
                            vscode.commands.executeCommand('pcg.openConfig');
                        }
                    });
                }
                this.isReady = false;
    ...
    private loadWorkspaceEnv(workspaceRoot: string): Record<string, string> {
        ...
        if (!fs.existsSync(envPath)) {
            this.outputChannel?.appendLine(
                ...
                `Run "PCG: Open Configuration" to create one.`
            );
            return {};
        }
                ...
                const response = await Promise.race([
                    ...
                    new Promise<never>((_, rej) =>
                        setTimeout(() => rej(new Error(
                            ...
                            `Check the "PlatformContextGraph" output channel for server logs.`
                        )), timeoutMs)
                    )
                ]);
     ...
     * The pcg MCP server writes all startup logs to stderr.
     */
    private _waitForProcessReady(timeoutMs: number): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            // pcg mcp start prints one of these when it is accepting requests:
            //   "Starting PlatformContextGraph Server"
            ...
            const onStderr = (chunk: Buffer) => {
                ...
                stderrAccum += text;
                this.outputChannel?.appendLine(`[PCG Server] ${text.trimEnd()}`);
            ...
            this.process.once('exit', (code) => {
                ...
                if (!resolved) {
                    const hint = stderrAccum.includes('neo4j:7687') || stderrAccum.includes('DNS')
                        ...
                        'Open "PCG: Open Configuration" and change NEO4J_URI to bolt://localhost:7687, ' +
                        'or switch DATABASE_TYPE to falkordb for zero-config local use.'
                        : ' Open "PCG: Open Configuration" to check your database settings.';
                    done(new Error(
                        `PCG server exited with code ${code} before it was ready.${hint}\n` +
            ...
            const timer = setTimeout(() => {
                    ...
                    `PCG server did not respond within ${timeoutMs / 1000}s.\n` +
                    `Last output: ${stderrAccum.slice(-300) || '(none — pcg binary may not be found)'}\n` +
                    `Try running "PCG: Open Configuration" to fix the path or database settings.`
                ));
            }, timeoutMs);
        });
    }
    ...
    // --- Helper Wrapper Methods matching previous PcgManager functionality ---
