...
export class PcgManager {
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        this.outputChannel = vscode.window.createOutputChannel("PlatformContextGraph");
        const pcgPath = this.findPcgExecutable();

        this.outputChannel.appendLine(`[Init] Initializing PcgManager`);
        this.outputChannel.appendLine(`[Init] Using pcg path: ${pcgPath}`);
        ...
        this.mcpClient = new McpClient(pcgPath, workspaceRoot, this.outputChannel);
    }

    /**
     * Find the pcg executable, checking virtual environments first
     */
    private findPcgExecutable(): string {
        const config = vscode.workspace.getConfiguration('pcg');
        const configuredPath = config.get<string>('cliPath') || config.get<string>('pcgPath');

        // If user has configured a specific path, use it
        if (configuredPath && configuredPath !== 'pcg') {
            this.outputChannel.appendLine(`[Init] Using configured pcg path: ${configuredPath}`);
            return configuredPath;
        }

        // Try to find pcg in virtual environments
        ...
        if (workspaceFolders && workspaceFolders.length > 0) {
            ...
            for (const name of venvNames) {
                ...
                const pcgInVenv = this.getPcgPathInVenv(venvPath);
                if (pcgInVenv && this.fileExists(pcgInVenv)) {
                    this.outputChannel.appendLine(`[Init] Found pcg in venv: ${pcgInVenv}`);
                    return pcgInVenv;
                }
            }
        }
        ...
        if (pythonPath) {
            const pcgInPythonEnv = this.getPcgPathFromPython(pythonPath);
            if (pcgInPythonEnv && this.fileExists(pcgInPythonEnv)) {
                this.outputChannel.appendLine(`Found pcg using Python interpreter: ${pcgInPythonEnv}`);
                return pcgInPythonEnv;
            }
        }

        // Fallback to system PATH
        this.outputChannel.appendLine('Using pcg from system PATH');
        return 'pcg';
    }

    /**
     * Get pcg path within a virtual environment
     */
    private getPcgPathInVenv(venvPath: string): string {
        const isWindows = process.platform === 'win32';
        if (isWindows) {
            return path.join(venvPath, 'Scripts', 'pcg.exe');
        } else {
            return path.join(venvPath, 'bin', 'pcg');
        }
    }

    /**
     * Get pcg path from Python interpreter path
     */
    private getPcgPathFromPython(pythonPath: string): string {
        const pythonDir = path.dirname(pythonPath);
        const isWindows = process.platform === 'win32';

        if (isWindows) {
            // On Windows, Scripts folder is at the same level as python.exe
            return path.join(pythonDir, 'pcg.exe');
        } else {
            // On Unix, bin folder contains both python and pcg
            return path.join(pythonDir, 'pcg');
        }
    }
    ...
    async getCallers(functionName: string): Promise<FunctionInfo[]> {
        try {
            this.outputChannel.appendLine(`[PcgManager] Fetching callers for: ${functionName}`);
            ...
            this.outputChannel.appendLine(`[PcgManager] Executing Cypher: ${cypherQuery}`);
            const response = await this.mcpClient.callTool('execute_cypher_query', { cypher_query: cypherQuery });
            this.outputChannel.appendLine(`[PcgManager] Cypher response: ${JSON.stringify(response, null, 2)}`);
    ...
    async getCallees(functionName: string): Promise<FunctionInfo[]> {
        try {
            this.outputChannel.appendLine(`[PcgManager] Fetching callees for: ${functionName}`);
