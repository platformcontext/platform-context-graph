...
"""
    ...
    assert any(
        m["name"] == "MyApp.Worker" and m["type"] == "defmodule" for m in modules
    )
    assert any(
        m["name"] == "MyApp.Serializable" and m["type"] == "defprotocol"
        for m in modules
    )
...
"""
    ...
    assert any(
        i["name"] == "MyApp.Repo" and i["import_type"] == "alias" for i in imports
    )
    assert any(
        i["name"] == "Ecto.Query" and i["import_type"] == "import" for i in imports
    )
