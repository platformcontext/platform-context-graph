...
class TestUserJourneys:
    ...
    These tests invoke the 'pcg' command line tool as a subprocess,
    simulating a real user interacting with the installed tool.
    """

    def run_pcg(self, args, cwd=None):
        """Helper to run the pcg CLI."""
        # Use sys.executable to ensure we use the same python environment
        import sys

        cmd = [sys.executable, "-m", "platform_context_graph.cli.main"] + args
    ...
    def test_first_time_user_workflow(self, python_sample_project, temp_test_dir):
        ...
        2. User runs 'pcg index' on verify basic project.
        3. User runs 'pcg list' to verify it's there.
        4. User runs 'pcg find function foo' to verify indexing worked.
        """
        ...
        result = self.run_pcg(["index", str(project_dir)])
        assert result.returncode == 0, f"Indexing failed: {result.stderr}"

        # 3. List
        result = self.run_pcg(["list"])
        ...
        # Correct command: pcg find name foo --type function
        result = self.run_pcg(["find", "name", "foo", "--type", "function"])
        assert result.returncode == 0
    ...
    def test_clean_up(self, temp_test_dir):
        ...
        self.run_pcg(["index", str(dummy_dir)])
        ...
        result = subprocess.run(
            [
                "python3",
                "-m",
                "platform_context_graph.cli.main",
                "delete",
                str(dummy_dir),
                "--yes",
            ],
