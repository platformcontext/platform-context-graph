...
def _seed_incremental_refresh_state(connection: psycopg.Connection[Any]) -> None:
    """Seed one active generation and one newer pending generation."""

    now = dt.datetime.now(dt.timezone.utc).replace(microsecond=0)
    observed_a = now - dt.timedelta(minutes=10)
    observed_b = now - dt.timedelta(minutes=1)
    graph_fact_payload = {
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            ...
            (
                ...
                observed_a,
                observed_a + dt.timedelta(minutes=1),
                "active",
                observed_a + dt.timedelta(minutes=1),
                json.dumps({"repo_id": _GRAPH_RECORD_ID, "generation": "A"}),
            ),
        )
        cursor.execute(
            """
            INSERT INTO scope_generations (
                ...
                %s, %s, %s, %s, %s, %s, %s, NULL, NULL, %s::jsonb
            )
            ON CONFLICT (generation_id) DO UPDATE SET
                scope_id = EXCLUDED.scope_id,
                trigger_kind = EXCLUDED.trigger_kind,
            ...
            (
                ...
                now,
                "pending",
                json.dumps({"repo_id": _GRAPH_RECORD_ID, "generation": "B"}),
            ),
        )
        cursor.execute(
            """
            ...
            """,
            (
                "fact-incremental-refresh-graph",
                _SCOPE_ID,
                _GENERATION_B_ID,
                "repository",
...
def _wait_for_refresh(
    ...
    *,
    ...
    """Poll until generation B becomes active and generation A is superseded."""
...
def test_incremental_refresh_compose(
    ...
    """Exercise generation replacement against compose-backed infrastructure."""

    _seed_incremental_refresh_state(connection)
