...
from .dbt_sql_lineage_resolution import (
...
def _lineage_for_projection(
    *,
    ...
    if match:
        ...
        output_column = implicit_output_column(expression)

    honesty_gap_reason = expression_honesty_gap_reason(expression)
    ...
    for reference in _QUALIFIED_REFERENCE_RE.finditer(expression):
        ...
        resolved_columns = resolve_reference_columns(
            binding,
            alias=alias,
            column=column,
            model_name=model_name,
    ...
    for identifier in unqualified_identifiers(
        ...
        resolved_columns = resolve_unqualified_reference_columns(
            identifier,
            relation_bindings=relation_bindings,
            model_name=model_name,
        )
