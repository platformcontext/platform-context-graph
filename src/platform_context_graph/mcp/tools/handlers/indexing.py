...
def add_code_to_graph(
    graph_builder, job_manager, loop, list_repos_func, **args
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for repo in indexed_repos:
            ...
            if repo_path and Path(repo_path).resolve() == path_obj:
                return {
                    "success": False,
        ...
        return {
            ...
            "estimated_duration_human": (
                f"{int(estimated_time // 60)}m {int(estimated_time % 60)}s"
                if estimated_time >= 60
                else f"{int(estimated_time)}s"
            ),
...
def add_package_to_graph(
    graph_builder, job_manager, loop, list_repos_func, **args: Any
) -> dict[str, Any]:
    ...
    if not isinstance(package_name, str) or not package_name.strip():
        return {"error": "The 'package_name' argument is required."}
    ...
    try:
        ...
        indexed_repos = list_repos_func().get("repositories", [])
        for repo in indexed_repos:
            if repo.get("is_dependency") and (
                repo.get("name") == package_name
                or repo.get("name") == f"{package_name}.py"
            ):
                return {
                    "success": False,
        ...
        if not package_path:
            return {
                "error": f"Could not find package '{package_name}' for language '{language}'. Make sure it's installed."
            }
        ...
        debug_log(
            f"Error creating background job for package {package_name}: {str(exc)}"
        )
        return {
            "error": f"Failed to start background processing for package '{package_name}': {str(exc)}"
        }
