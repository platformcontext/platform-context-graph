"""Tree-sitter parser support for Rust."""
...
class RustTreeSitterParser:
    ...
    def __init__(self, generic_parser_wrapper: Any):
        """Initialize the Rust parser with the shared tree-sitter wrapper."""
    ...
    def _get_parent_context(
        ...
        """Return the nearest enclosing Rust declaration."""
        curr = node.parent
        while curr:
    ...
    def _find_functions(self, root_node: Any) -> list[Dict[str, Any]]:
        """Find Rust function declarations."""
        functions = []
    ...
    def _find_structs(self, root_node: Any) -> list[Dict[str, Any]]:
        """Find Rust struct, enum, and trait declarations."""
        structs = []
        query_str = """
        [
            (struct_item) @s
                ...
                calls.append(
                    {
                        "name": call_name,
                        "full_name": (
                            self._get_node_text(call_node) if call_node else call_name
                        ),
                        "line_number": node.start_point[0] + 1,
                        "args": args,
