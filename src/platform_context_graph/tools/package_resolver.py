"""Helpers for resolving local package installation paths by language."""
...
def _get_go_package_path(package_name: str) -> Optional[str]:
    ...
    def _first_existing_dir(output: str) -> Optional[str]:
        """Return the first existing directory listed in command output."""
...
def _get_php_package_path(package_name: str) -> Optional[str]:
    """Find the local installation path of a Composer package."""
    try:
...
def _get_dart_package_path(package_name: str) -> Optional[str]:
    ...
    try:
        ...
        if hosted_path.exists():
            ...
            versions = [
                d for d in hosted_path.parent.glob(f"{package_name}-*") if d.is_dir()
            ]
            if versions:
...
def get_local_package_path(package_name: str, language: str) -> Optional[str]:
    """Dispatch to the correct package path finder for a language."""
    finders = {
...
def _get_cpp_package_path(package_name: str) -> Optional[str]:
    """Find the local installation path of a C++ package."""
    import subprocess
    import os
