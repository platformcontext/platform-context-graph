# src/platform_context_graph/core/database.py
"""
...
def _query_kind(query: str) -> str:
    """Return a low-cardinality operation name for a Cypher query."""
    tokens = query.strip().split(None, 1)
    if not tokens:
        return "unknown"
...
class InstrumentedSession:
    """Wrap a Neo4j session to emit low-cardinality query metrics and spans."""

    def __init__(self, session):
        """Wrap an active Neo4j session object."""
        self._session = session

    def __enter__(self):
        """Enter the wrapped Neo4j session context."""
        self._session.__enter__()
        return self

    def __exit__(self, exc_type, exc, tb):
        """Exit the wrapped Neo4j session context."""
        return self._session.__exit__(exc_type, exc, tb)

    def run(self, query, *args, **kwargs):
        """Execute a Cypher query while recording telemetry."""
        runtime = get_observability()
        query_kind = _query_kind(str(query))
    ...
    def __getattr__(self, name):
        """Delegate missing attributes to the wrapped session."""
...
class Neo4jDriverWrapper:
    ...
    def __init__(self, driver: Driver, database: str = None):
        """Wrap a Neo4j driver and optionally pin it to one database."""
        self._driver = driver
    ...
    def session(self, **kwargs):
        ...
        return InstrumentedSession(self._driver.session(**kwargs))
    ...
    def __getattr__(self, name):
        """Delegate unknown attributes to the underlying Neo4j driver."""
...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        self.neo4j_database = os.getenv(
            "NEO4J_DATABASE"
        )  # Optional, if not set, will use default database configured in Neo4j
    ...
    def validate_config(
        ...
        uri_pattern = (
            r"^(neo4j|neo4j\+s|neo4j\+ssc|bolt|bolt\+s|bolt\+ssc)://[^:]+(:\d+)?$"
        )
    ...
    @staticmethod
    def test_connection(
        uri: str, username: str, password: str, database: str = None
    ) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            session_kwargs = {}
            if database:
                session_kwargs["database"] = (
                    database  # Pass database to session if provided
                )
            ...
            if "authentication" in error_msg or "unauthorized" in error_msg:
                ...
                or "failed to establish connection" in error_msg
            ):
                return False, (
