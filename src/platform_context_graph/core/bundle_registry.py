"""GitHub-backed bundle registry client helpers."""
...
class BundleRegistry:
    """
    Core logic for interacting with the PlatformContextGraph bundle registry.
                ...
                if manifest.get("bundles"):
                    for bundle in manifest["bundles"]:
                        ...
                        if "bundle_name" in bundle:
                            # Extract full name without .pcg extension
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                weekly_releases = [
                    r
                    for r in releases
                    if r["tag_name"].startswith("bundles-")
                    and r["tag_name"] != "bundles-latest"
                ]
                    ...
                    for asset in latest_weekly.get("assets", []):
                        if asset["name"].endswith(".pcg"):
                            ...
                            bundle = {
                                ...
                                "version": (
                                    name_parts[1] if len(name_parts) > 1 else "latest"
                                ),
                                "commit": (
                                    name_parts[2] if len(name_parts) > 2 else "unknown"
                                ),
            ...
            if "name" not in bundle:
                ...
                if "/" in repo:
                    ...
                    full_name = bundle.get(
                        "full_name", bundle.get("bundle_name", "unknown")
                    )
