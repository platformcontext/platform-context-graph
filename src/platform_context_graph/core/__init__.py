# src/platform_context_graph/core/__init__.py
"""
...
def get_database_manager() -> (
    Union[
        "DatabaseManager", "FalkorDBManager", "FalkorDBRemoteManager", "KuzuDBManager"
    ]
):
    """
    ...
    Selection logic:
    1. Runtime Override: 'PCG_RUNTIME_DB_TYPE' (set via --database flag)
    2. Configured Default: 'DEFAULT_DATABASE' (set via 'pcg default database')
    ...
    db_type = os.getenv("PCG_RUNTIME_DB_TYPE")
    if not db_type:
    ...
    if db_type:
        ...
        if db_type == "kuzudb":
            if not _is_kuzudb_available():
                raise ValueError(
                    "Database set to 'kuzudb' but Kùzu is not installed.\nRun 'pip install kuzu'"
                )
            ...
            if not _is_falkordb_available():
                info_logger(
                    "FalkorDB Lite is not supported or not installed. Falling back to KùzuDB."
                )
            ...
            try:
                ...
                info_logger(
                    f"FalkorDB Lite not functional ({falkor_err}). Falling back to KùzuDB."
                )
                if _is_kuzudb_available():
                    from .database_kuzu import KuzuDBManager
            ...
            raise ValueError(
                f"Unknown database type: '{db_type}'. Use 'kuzudb', 'falkordb', 'falkordb-remote', or 'neo4j'."
            )
    ...
    error_msg += "Alternative: Run 'pcg neo4j setup' to configure Neo4j."
