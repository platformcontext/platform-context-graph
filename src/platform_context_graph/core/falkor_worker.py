"""Standalone FalkorDB worker process for embedded deployments."""
...
def handle_signal(signum, frame):
    """Exit the worker cleanly when it receives a shutdown signal."""
    del frame
...
def run_worker():
    """Start and supervise the FalkorDB Lite worker process."""
    global db_instance
    ...
    if not db_path or not socket_path:
        logger.error(
            "Missing configuration. FALKORDB_PATH and FALKORDB_SOCKET_PATH must be set."
        )
        sys.exit(1)
    ...
    try:
        ...
        db_instance = FalkorDB(
            db_path, unix_socket_path=socket_path, serverconfig=server_config
        )
        logger.info("FalkorDB Lite is running.")

        # Validate that FalkorDB module actually loaded by running a GRAPH.QUERY
        try:
            test_graph = db_instance.select_graph("__pcg_health_check")
            ...
            if "graph.query" in err_str or "unknown command" in err_str:
                ...
                logger.warning(
                    f"FalkorDB health-check warning (non-fatal): {health_err}"
                )
