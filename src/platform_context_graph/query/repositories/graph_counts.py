...
def repository_graph_counts(session: Any, repo: dict[str, Any]) -> dict[str, int]:
    ...
    row = session.run(
        ...
        CALL (r) {{
            OPTIONAL MATCH (r)-[:REPO_CONTAINS]->(file:File)
            RETURN count(DISTINCT file) AS file_count
        }}
        CALL (r) {{
            OPTIONAL MATCH (r)-[:REPO_CONTAINS]->(:File)-[:CONTAINS]->(fn:Function)
            WHERE NOT EXISTS {{
        ...
        CALL (r) {{
            OPTIONAL MATCH (r)-[:REPO_CONTAINS]->(:File)-[:CONTAINS]->(:Class)-[:CONTAINS]->(fn:Function)
        ...
        CALL (r) {{
            OPTIONAL MATCH (r)-[:REPO_CONTAINS]->(:File)-[:CONTAINS*]->(cls:Class)
            RETURN count(DISTINCT cls) AS class_count
        }}
        CALL (r) {{
            OPTIONAL MATCH (r)-[:REPO_CONTAINS]->(:File)-[rel]->(module:Module)
