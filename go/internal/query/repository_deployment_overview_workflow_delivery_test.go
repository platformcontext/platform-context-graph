...
`,
	...
	if got, want := topologyStory[0], "Workflow delivery paths include .github/workflows/deploy-platform.yml as github_actions_workflow deploy-platform triggered by workflow_dispatch with 4 run command(s) and delivery families docker_compose, helm, kubectl, terraform (workflow_file, run_commands, workflow_triggers, delivery_command_families)."; got != want {
