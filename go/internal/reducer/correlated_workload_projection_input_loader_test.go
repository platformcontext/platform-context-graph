...
func TestCorrelatedWorkloadProjectionInputLoaderRejectsLowConfidenceDockerfileOnlyCandidate(t *testing.T) {
	t.Parallel()
	...
	if len(candidates) != 0 {
		t.Fatalf("len(candidates) = %d, want 0 for low-confidence Dockerfile-only evidence", len(candidates))
	}
}
