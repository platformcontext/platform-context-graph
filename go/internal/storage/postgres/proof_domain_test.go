...
func TestProofDomainIncrementalRefreshRetriesChangedGenerationBeforePromotion(t *testing.T) {
	t.Parallel()
	...
	runProofProjectorCycleWithInjector(t, db, now, injector)

	if got, want := db.activeGenerationID(scopeValue.ScopeID), generationA.GenerationID; got != want {
		t.Fatalf("active generation after retryable failure = %q, want %q", got, want)
	}
	if got, want := db.generationStatus(generationA.GenerationID), scope.GenerationStatusActive; got != want {
		t.Fatalf("generation A status after retryable failure = %q, want %q", got, want)
	}
	if got, want := db.generationStatus(generationB.GenerationID), scope.GenerationStatusPending; got != want {
		t.Fatalf("generation B status after retryable failure = %q, want %q", got, want)
	}
	if got, want := db.projectorWorkItemStatus(generationB.GenerationID), "retrying"; got != want {
		t.Fatalf("generation B projector status after retryable failure = %q, want %q", got, want)
	}

	runProofProjectorCycleWithInjector(t, db, now.Add(2*time.Second), injector)

	if got, want := db.activeGenerationID(scopeValue.ScopeID), generationB.GenerationID; got != want {
		t.Fatalf("active generation after retry recovery = %q, want %q", got, want)
	}
	if got, want := db.generationStatus(generationA.GenerationID), scope.GenerationStatusSuperseded; got != want {
		t.Fatalf("generation A status after retry recovery = %q, want %q", got, want)
	}
	if got, want := db.generationStatus(generationB.GenerationID), scope.GenerationStatusActive; got != want {
		t.Fatalf("generation B status after retry recovery = %q, want %q", got, want)
	}
	if got, want := db.projectorWorkItemStatus(generationB.GenerationID), "succeeded"; got != want {
		t.Fatalf("generation B projector status after retry recovery = %q, want %q", got, want)
	}
}
