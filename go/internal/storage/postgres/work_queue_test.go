...
func TestProjectorQueueEnqueueWithFactCountStoresSchedulingPayload(t *testing.T) {
	t.Parallel()
	...
	err := queue.EnqueueWithFactCount(
		context.Background(),
		"scope-123",
		"generation-456",
		148948,
	...
	if err != nil {
		t.Fatalf("EnqueueWithFactCount() error = %v, want nil", err)
	}
	if got, want := len(db.execs), 1; got != want {
		t.Fatalf("exec count = %d, want %d", got, want)
	}
	if !strings.Contains(db.execs[0].query, "$6::jsonb") {
		t.Fatalf("enqueue query = %q, want jsonb payload argument", db.execs[0].query)
	}
	payloadBytes, ok := db.execs[0].args[5].([]byte)
	if !ok {
		t.Fatalf("payload arg type = %T, want []byte", db.execs[0].args[5])
	}
	payload := string(payloadBytes)
	for _, want := range []string{`"fact_count":148948`, `"repo_size_tier":"large"`} {
...
func TestProjectorQueueClaimCanPreferLargeGenerationPayloads(t *testing.T) {
	t.Parallel()
		...
		queryResponses: []queueFakeRows{
			{
				rows: [][]any{{
					...
					"snapshot",
					"",
					[]byte(`{"repo_id":"repository:r_test","fact_count":148948}`),
				}},
			},
		},
	}
	...
	queue := ProjectorQueue{
		...
		LeaseOwner:                  "projector-1",
		LeaseDuration:               30 * time.Second,
		PreferLargeGenerationsFirst: true,
		Now:                         func() time.Time { return now },
	}
	...
	if got, want := db.queries[0].args[3], true; got != want {
		t.Fatalf("prefer large arg = %v, want %v", got, want)
	}
}
