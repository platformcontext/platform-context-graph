# PlatformContextGraph Troubleshooting Guide

Use this checklist whenever `pcg neo4j setup` or `pcg start` doesn’t behave as expected. It keeps the happy path short, but includes the fallback steps when something goes wrong.
...
- **Docker Desktop** installed *and running* before you launch `pcg neo4j setup` if you want the wizard to spin up a local Neo4j for you.
...
From the repository root (`PlatformContextGraph/`):
...
```powershell
.\venv\Scripts\pcg.exe neo4j setup
```
...
- At the end, it generates:
  ...
  - `~/.platform-context-graph/.env` containing `NEO4J_URI`, `NEO4J_USERNAME`, `NEO4J_PASSWORD`.
...
2. (Optional) Also create `%USERPROFILE%\.platform-context-graph\.env` with the same key/value pairs. The CLI loads that file automatically.
...
| --- | --- | --- |
| `Configuration Error: Neo4j credentials must be set…` | `mcp.json`/`.env` missing or empty | Run `pcg neo4j setup` again **with Docker running**, or create the files manually (section 5). |
...
| Setup wizard fails while pulling Docker image | Docker Desktop not running or Docker permissions missing | Start Docker Desktop, wait for it to report “Running”, then rerun `pcg neo4j setup`. |
| Server exits immediately with no log | Neo4j instance is offline | Check Docker container status or AuraDB dashboard; restart Neo4j and call `pcg start` again. |

## 7. After the server is running

- Keep the virtual environment active whenever you run `pcg` commands.
...
- Product and reference docs live under `docs/docs/`.
