...
| Product-truth fixture contract | Started | `tests/fixtures/product_truth/manifest.json` now registers owned graph-analysis, correlation-DSL, and relationship-platform suites plus the planned `iac_quality.dead_iac` capability. `tests/fixtures/product_truth/dead_iac/` adds generic Terraform, Helm, and Ansible fixture repos with used, unused, and ambiguous cases, and `tests/fixtures/product_truth/expected/dead_iac.json` records the expected truth assertions. `scripts/verify_product_truth_fixtures.sh` verifies the corpus and expected assertions statically. Commit `17af2941` added the fixture corpus. | Promote dead-IaC from planned to owned by adding runtime API/MCP/Postgres evidence assertions and wiring the compose verifier to execute those golden expectations. |
| Phase 1 API candidate surface | Started | `POST /api/v0/iac/dead` now exposes a bounded `derived` candidate surface for explicit `repo_id`/`repo_ids` scopes. Tests prove Terraform, Helm, and Ansible used artifacts are not returned, unused artifacts return `candidate_dead_iac`, and dynamic references return `ambiguous_dynamic_reference` when requested. | Replace query-time content scanning with reducer-materialized IaC usage/finding rows, add Postgres evidence assertions, then mirror the proven API through MCP. |
| Phase 1 materialized storage | Started | `iac_reachability_rows` bootstrap schema and `IaCReachabilityStore` now persist used, unused, and ambiguous IaC artifact rows with evidence and limitations JSON. Focused tests prove cleanup queries exclude used artifacts and only include ambiguous rows when requested. | Wire reducer materialization to write these rows from fixture-backed IaC references, then switch `/api/v0/iac/dead` to prefer durable rows over query-time scanning. |
