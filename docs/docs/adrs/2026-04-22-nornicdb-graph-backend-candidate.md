...
- the first rerun with the batched semantic writer still timed out after `18.140664853s`, which narrowed the remaining reducer issue to semantic statement width. Current branch now adds `PCG_NORNICDB_SEMANTIC_ENTITY_LABEL_BATCH_SIZES=Function=15,Variable=10,...` for the high-cardinality semantic labels and includes the semantic label/row count in timeout errors so future failures are diagnosable without another blind rerun.
