...
- current branch now keeps the split node-upsert / containment-edge shape only for backends that support node-only batched `MERGE`, and routes NornicDB through the proven file-scoped combined shape: match the `File` anchor with `$file_path`, unwind entity rows for that file, upsert nodes, and attach `CONTAINS` in one statement. The opt-in syntax gate now records why this is necessary: the current NornicDB binary collapses the standalone node-only batch shape, while the combined shape preserves row-bound entity identity.
