...
- the follow-up with semantic `Function=10` exposed one more high-width semantic family: `Annotation rows=100` still reached the `30s` write budget at `1859s`. Current branch narrows only the NornicDB semantic `Annotation` cap to `50`, keeps `PCG_CANONICAL_WRITE_TIMEOUT` configurable, and updates the remote harness to test the same semantic cap baseline that ships in code.
