...
- current branch adds `pcg index --discovery-report <file>` and the
  ...
  directories/files, entity counts, and skip breakdowns. This is intentionally
  an operator artifact rather than high-cardinality telemetry, and it supports
  the current policy of using `.pcg/discovery.json` for ambiguous repo-local
  archive/vendor decisions instead of baking broad global skips.
