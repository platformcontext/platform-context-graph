...
- the next performance gate should intentionally repeat the 20-repo corpus with continuous CPU, disk, and queue sampling before another full-corpus burn. If resource utilization drops while a few graph writes remain in flight, the optimization target is conflict-domain routing or a NornicDB hot-path patch, not more workers, lower worker counts, or another blind timeout/row-cap change.
