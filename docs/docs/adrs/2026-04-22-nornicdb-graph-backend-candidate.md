...
Latest 2026-04-24 NornicDB dogfood evidence:
...
- grouping canonical entity batches by `label + file_path` and matching the file anchor once per statement is a real improvement too: the fresh rebuilt-binary rerun now keeps deep `Function rows=15` chunks in roughly the `0.3s-1.9s` band instead of the earlier `2s-9s+` range
