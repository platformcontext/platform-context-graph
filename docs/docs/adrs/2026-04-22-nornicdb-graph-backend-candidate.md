...
- 2026-05-04 Neo4j comparison snapshot and correction
  ...
  this manual Neo4j runner skipped `pcg-bootstrap-data-plane`, so it had no
  graph schema bootstrap. Corrected schema-first run
  `pcg-neo4j-schema-profile-20260504T182712Z` applied graph schema before
...
- Neo4j remains strategically useful for companies that already license and
  ...
  prove the same PCG truth contract without handler-level workarounds. If PCG
