...
- the promotion-relevant remote full-corpus lane now uses PCG `b6087e2c` with the combined PR `#119` / `#120` NornicDB headless binary at `86e78f1`. It passed self-repo in `20s`, passed focused Crossplane in `5s`, and completed all `896` repos in `/home/ubuntu/pcg-e2e-full` in `642s` with `final_status=healthy`. The run proves the baked NornicDB reducer default (`workers=1`), the generated-webpack filter (`files_skipped.content.generated-webpack`), and the legacy vendored-library filters (`files_skipped.content.vendored-zend-framework`, `files_skipped.content.vendored-browser-library`, `files_skipped.content.vendored-fpdf`) are active under the production-scale corpus. That run also exposed harmless shutdown `context canceled` projector noise after the harness stopped the already-healthy owner; current branch now treats owner-context cancellation during fact load/projection as shutdown instead of failing the durable projector work item.
