...
- the next timing pass answered the timeout-policy question directly: `PCG_CANONICAL_WRITE_TIMEOUT=30s` moved the full-corpus wall from `1163s` to `1694s`, proving the knob works and the new error hint is actionable, but the run still failed on a much wider semantic statement (`Annotation rows=500`). Current branch therefore promotes `30s` as the NornicDB default and caps semantic `Annotation` rows at `100` by default instead of treating a larger timeout as the only fix.
