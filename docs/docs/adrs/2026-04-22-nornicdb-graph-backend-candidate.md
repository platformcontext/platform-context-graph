...
- the follow-up with semantic `Function=10` exposed one more high-width semantic family: `Annotation rows=100` still reached the `30s` write budget at `1859s`, so the branch narrowed Annotation to `50`. The next full-corpus pass crossed the prior wall and reached `504/896` projector scopes before `Annotation rows=50` failed at `1849s`; several successful `rows=50` statements were already in the `23s-25s` band. Current branch therefore narrows only the NornicDB semantic `Annotation` cap to `25`, keeps `PCG_CANONICAL_WRITE_TIMEOUT` configurable, and updates the remote harness to test the same semantic cap baseline that ships in code.
