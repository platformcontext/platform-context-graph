...
- The current remote full-corpus run `/tmp/pcg-perf-20260426T112652Z-relset-hotpath` uses PCG `9e2bc7e3` with NornicDB `3239983`. It passed the self-repo lane in `20s`, the focused `crossplane-xrd-irsa-role` lane in `5s`, crossed the old `90s` failure wall, and was still progressing at `16m44s` with projector `succeeded=247`, reducer `succeeded=1812`, and `dead_letter=0`. Early logs show the hot-path class change clearly: many file-scoped entity chunks now complete in milliseconds while remaining singleton and semantic statements expose the next targeted bottleneck. This preserves the compatibility policy: patch NornicDB for performant Neo4j-equivalent primitives before adding PCG-side global graph-write throttles.
