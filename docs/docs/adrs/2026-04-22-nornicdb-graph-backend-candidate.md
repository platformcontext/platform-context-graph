...
- upstream status as of 2026-04-24: the unique-constraint validation fix is now a clean upstream PR at `https://github.com/orneryd/NornicDB/pull/115`, and the `UNWIND ... MATCH ... MERGE ... SET n += row.props ... MERGE relationship` hot-path/router fix is a separate clean upstream PR at `https://github.com/orneryd/NornicDB/pull/116`. PCG should keep using explicit `--from`/manifest-pinned binaries for local-authoritative dogfood until both fixes are merged, released, and pinned; after that, remove the temporary fork-binary language from the promotion gate and rerun the self-repo dogfood lane against the release-backed headless asset.
- 2026-04-25 follow-up upstream handoff adds `https://github.com/orneryd/NornicDB/pull/118`, which teaches NornicDB `MERGE` lookup to use single-property unique-constraint metadata instead of falling back to label scans. Copilot review comments on PRs `#116` and `#118` are addressed in the fork branches: `#116` now counts only rows that survive required `MATCH` lookups and falls back safely for unsupported `OPTIONAL MATCH` mutation shapes; `#118` now treats non-comparable unique lookup values as lookup misses instead of panicking.
