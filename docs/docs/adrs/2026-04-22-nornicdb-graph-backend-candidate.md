...
- the branch now emits rolling and final `nornicdb entity label summary` logs with per-label rows, statements, executions, grouped chunks, total duration, max execution duration, and row-width totals so the next tuning slice can explain cumulative label cost before changing more defaults
