...
| --- | --- | --- | --- |
| Dead-IaC product truth | In progress | Commit `Add dead-IaC reachability query proof` extracts the dead-IaC analyzer into `go/internal/iacreachability`, proves Terraform/Helm/Ansible used, unused, and ambiguous classifications against `tests/fixtures/product_truth/dead_iac`, teaches `/api/v0/iac/dead` to prefer reducer-materialized Postgres reachability rows when present, keeps the bounded content fallback for fixture/local use, and exposes MCP `find_dead_iac`. Follow-up implementation `Materialize dead-IaC reachability rows after projection` wires bootstrap finalization to materialize active-corpus IaC reachability rows after source-local projection drains, adds `pcg_dp_iac_reachability_materialization_duration_seconds` and `pcg_dp_iac_reachability_rows_total`, and prevents the API from falling back to derived content analysis when materialized rows exist but contain zero cleanup findings. Local verification: `go test ./internal/iacreachability ./internal/mcp ./internal/query ./internal/storage/postgres ./internal/telemetry ./cmd/api ./cmd/mcp-server ./cmd/bootstrap-index -count=1`, `scripts/verify_product_truth_fixtures.sh`, `git diff --check`, `uv run --with mkdocs --with mkdocs-material --with pymdown-extensions mkdocs build --strict --clean --config-file docs/mkdocs.yml`, and rebuilt local binaries. Classification: correctness/materialization work, not a reducer throughput win. | Push the materialization slice, pull it on the remote machine, rebuild Linux binaries, run the dead-IaC fixture/runtime proof remotely, and verify API/MCP output against the expected product-truth assertions. |
