...
| Communicator ArgoCD control-plane follow-up | Investigating | Rebuilt remote binaries at `b937650d` and reran a fresh no-symlink `11`-repository corpus that added `iac-eks-argocd` and core automation to the previous communicator, Terraform, config, workflow-library, and `helm-charts` set. Run `pcg-focused-communicator-dual-20260430Tnow` drained healthy with projector `11/11`, reducer `98/98`, and `0` failed/dead-lettered items. Manual finalization measured about `146s` because the temporary harness had a SQL quoting bug after the queue was already healthy; reducer handler evidence was small (`workload_materialization` handler sum `2.160s`, max `0.223s`; `deployment_mapping` sum `0.961s`, max `0.140s`) and resource monitoring stayed idle-heavy. Durable evidence now includes `49` `ARGOCD_APPLICATIONSET_DISCOVERY` facts resolving to one `DISCOVERS_CONFIG_IN` relationship for `helm-charts`, plus the same Terraform provisioning chain into `api-node-communicator` (`31` facts) and `lambda-api-communicator` (`4` facts). It does **not** emit `ARGOCD_APPLICATIONSET_DEPLOY_SOURCE` for this templated ApplicationSet/config.yaml shape. Direct persisted-graph inspection confirmed `Repository` edges `DEPENDS_ON=1`, `DEPLOYS_FROM=5`, `DISCOVERS_CONFIG_IN=1`, `PROVISIONS_DEPENDENCY_FOR=3`; one `Workload` for `api-node-communicator`; and still `0` matching `WorkloadInstance` / `RUNS_ON` rows. Source inspection found the service name in the shared platform chart config and explicit Terraform runtime/provisioning evidence, but no direct service-named ApplicationSet in the included ArgoCD control repo. | Corrects the previous corpus conclusion: the 9-repo proof did not include the ArgoCD control-plane repo, so it could not validate Kubernetes dual-deployment truth. Next step is TDD for the templated ApplicationSet/config.yaml extraction gap, but only for cases where config data explicitly identifies the deployed source/workload; do not infer `api-node-communicator` runtime instances from shared platform chart client/config entries alone. |
...
| Full-corpus acceptance | In progress | Baseline `7h43m40s` was unacceptable. Current rebuilt full-corpus proof `pcg-reducer-full-after-disk-clean-815-20260429T194528Z` drained healthy in `931s` with projector `896/896`, reducer `8284/8284`, `code_calls 118564/118564`, `repo_dependency 2057/2057`, and no failed/dead-lettered work. Resource summary still showed headroom (`cpu_idle_avg=42.16%`, `io_wait_avg=2.65%`, `disk_idle_avg=81.29%`). | This meets the under-2-hour reducer target for the current proof shape. Next acceptance work is graph-truth/API validation plus targeted `workload_materialization` and `deployment_mapping` handler/data-shape optimization. |
