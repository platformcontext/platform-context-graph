...
| --- | --- | --- | --- |
| Dead-IaC product truth | In progress | Commit `Add dead-IaC reachability query proof` extracts the dead-IaC analyzer into `go/internal/iacreachability`, proves Terraform/Helm/Ansible used, unused, and ambiguous classifications against `tests/fixtures/product_truth/dead_iac`, teaches `/api/v0/iac/dead` to prefer reducer-materialized Postgres reachability rows when present, keeps the bounded content fallback for fixture/local use, and exposes MCP `find_dead_iac`. Local verification: `go test ./internal/iacreachability -count=1`, `go test ./internal/mcp ./internal/query ./internal/storage/postgres ./cmd/api -count=1`. Classification: correctness/query-surface work, not a reducer throughput win. | Rebuild binaries, push the branch, run the dead-IaC fixture proof on the remote machine, and verify API/MCP output against the expected product-truth assertions. |
