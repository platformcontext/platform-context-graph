...
| Semantic source-local drain gate | Pending remote proof | Current slice adds TDD coverage and claim-query gating so `semantic_entity_materialization` waits until all source-local projector work has drained when the existing NornicDB/local-authoritative projector-drain gate is enabled. The scoped same-`scope_id` reducer gate remains in place for all domains, and non-semantic reducer domains can still drain once their own source has passed the same-scope gate. Local verification passed `go test ./internal/storage/postgres -count=1`, `go test ./cmd/reducer ./internal/reducer ./internal/storage/postgres -count=1`, and `go test ./cmd/ingester ./internal/projector ./internal/storage/postgres -count=1`. | Correctness-preserving scheduling hypothesis pending full-corpus proof. Expected signal: semantic reducers stay pending while source-local is active, source-local contention becomes measurable without semantic retry noise, and the run must finish at or below the `35m` benchmark to be accepted. |
