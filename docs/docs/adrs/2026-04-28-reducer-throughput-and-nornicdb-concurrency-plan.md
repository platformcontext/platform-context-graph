...
| Multi-valued deployment and platform materialization | Correctness fix; remote proof pending | The live full-corpus graph audit after `5171441e` showed the next truth issue: repository-level relationship edges preserved multiple verbs and evidence pointers, but workload materialization still had scalar assumptions. A workload candidate carried only one `DeploymentRepoID`, deployable-unit correlation emitted one deployment-repo evidence atom, and provisioned runtime platform projection skipped an infrastructure repo unless it had exactly one `PROVISIONS_PLATFORM` row. That is not compatible with real migrations where `current`, `poc`, or `new` environments may use different deployment controllers, deployment repos, or runtime platforms. The in-progress local patch adds `DeploymentRepoIDs` as the additive list while keeping `DeploymentRepoID` as a compatibility primary, projects all deployment repositories that apply to each environment, keeps every deployment repo in deployable-unit correlation evidence, and emits all proven provisioned platform rows once runtime-specific provisioning evidence exists. Focused local tests now cover multiple deployment repos, multiple deployment environments, and multiple provisioned ECS/EKS-style platform rows while preserving the generic/ambiguous provisioning negative cases. Local verification passed `go test ./internal/reducer ./cmd/reducer ./internal/query -count=1` and `git diff --check`. | Rebuild and replay a focused workload-materialization proof on the running full-corpus stack, then query raw graph JSON for `DEPLOYMENT_SOURCE` and multiple `RUNS_ON` rows before classifying the slice as proven. |
| Nested filesystem repository discovery | Correctness fix; remote proof pending | A focused full-corpus evidence audit found that a platform grouping directory with visible metadata files was being selected as one tiny synthetic repository while its nested real Git checkouts were skipped. That made GitOps and Helm source files invisible to fact emission, relationship extraction, reducer materialization, Postgres read models, and graph truth even though the files existed on disk. Commit `b4d156cb` adds the local regression `TestDiscoverRepoRootsPrefersNestedGitReposOverVisibleGroupFiles`, which reproduces this shape generically and the collector now prefers descendant Git-backed repositories over parent grouping directories that merely contain visible files. Commit `2e50e37c` adds the hidden-state-directory regression so group-local dot directories do not become synthetic repositories. Focused local verification passed `go test ./internal/collector ./internal/collector/discovery -count=1`. | Commit, push, pull to the remote proof host, rebuild binaries, and run a focused discovery/evidence check proving the nested GitOps and Helm repos are selected as independent repositories before rerunning deployment-story materialization. |
| ApplicationSet service-identity deploy evidence | Correctness fix; remote proof pending | A focused source audit found a valid GitOps shape that PCG treated as discovery-only: the ApplicationSet discovers generator config in a values/config repo, the rendered sources point at a shared chart plus the same config repo, and the deployed service is identified by config fields such as `addon` or Helm `releaseName` rather than by a service `repoURL`. Commit `aafa414a` adds the local regression `TestDiscoverArgoCDApplicationSetDeploySourceFromConfigServiceIdentity`, which proves this generic shape while `TestDiscoverArgoCDApplicationSetSharedChartConfigDoesNotInventDeploySource` keeps arbitrary client/config references from becoming deploy-source truth. | Commit, push, pull to the remote proof host, rebuild binaries, and run a focused evidence extraction check against the real nested GitOps/Helm repos before a fresh focused corpus run. |
| Templated ApplicationSet destination guard | Correctness fix; remote proof pending | The focused real-source extractor check after `aafa414a` emitted the desired `DEPLOYS_FROM` evidence, but also exposed an invalid `RUNS_ON` platform built from an unresolved destination template. Commit `c05e1257` adds the local regression `TestDiscoverArgoCDApplicationSetSkipsTemplatedDestinationPlatform` now keeps unresolved `{{...}}` destination names or servers as provenance-only until concrete cluster data is available. | Commit, push, pull to the remote proof host, rebuild binaries, and rerun the focused evidence extraction check; expected output is `DISCOVERS_CONFIG_IN` plus `DEPLOYS_FROM`, with no templated platform edge. |
