...
| Nested filesystem repository discovery | Correctness fix; remote proof pending | A focused full-corpus evidence audit found that a platform grouping directory with visible metadata files was being selected as one tiny synthetic repository while its nested real Git checkouts were skipped. That made GitOps and Helm source files invisible to fact emission, relationship extraction, reducer materialization, Postgres read models, and graph truth even though the files existed on disk. The patch adds the local regression `TestDiscoverRepoRootsPrefersNestedGitReposOverVisibleGroupFiles`, which reproduces this shape generically and the collector now prefers descendant Git-backed repositories over parent grouping directories that merely contain visible files. Focused local verification passed `go test ./internal/collector ./internal/collector/discovery -count=1`. | Commit, push, pull to the remote proof host, rebuild binaries, and run a focused discovery/evidence check proving the nested GitOps and Helm repos are selected as independent repositories before rerunning deployment-story materialization. |
