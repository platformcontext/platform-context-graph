...
| n/a | Test `PCG_LARGE_GEN_MAX_CONCURRENT=4` without changing reducer workers | Remove source-local large-generation semaphore wait while preserving reducer readiness gates | Same hot20 corpus and same PCG/NornicDB heads moved `307s` to `240s`; all `20` projector and `160` reducer items succeeded; CPU idle fell `69.77%` to `62.91%`, IO wait stayed low at `2.06%`, disk idle stayed high at `82.07%`; large-generation semaphore acquisitions were immediate | Real 20-repo wall-clock and scheduling/admission win; candidate local-authoritative default proof, not a reducer worker-count increase |
...
those repos, while CPU and disk still have headroom.
