...
| Endpoint merge lookup index | Implemented locally; runtime proof pending | Commit `e7c42272` adds the missing NornicDB explicit `Endpoint.id` lookup index. TDD first failed `TestSchemaStatementsForBackendAddsNornicDBMergeLookupIndexes` because the NornicDB schema rendered no `Endpoint(id)` lookup index even though workload endpoint materialization uses `MERGE (endpoint:Endpoint {id: row.endpoint_id})` and subsequent `MATCH (endpoint:Endpoint {id: row.endpoint_id})` statements. The implementation adds only the backend-specific DDL entry and updates graph-backend operations docs. Local verification passed `go test ./internal/graph -run TestSchemaStatementsForBackendAddsNornicDBMergeLookupIndexes -count=1`, `go test ./internal/graph -count=1`, and `PCG_NORNICDB_BINARY=<upstream-main-nornicdb-build> go test ./cmd/pcg -run TestNornicDBSchemaAdapterVerification -count=1 -v` against upstream-main NornicDB `v1.0.43`. Classification: evidence-backed query/index-shape fix, not yet a wall-clock win. | Commit, push, rebuild remote PCG binaries, rerun the endpoint-heavy proof against upstream-main NornicDB `v1.0.43`, and compare endpoint graph-write durations before any full-corpus run. Acceptance requires materially lower endpoint write duration and direct graph/API truth unchanged. |
