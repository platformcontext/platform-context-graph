...
edge batch duration should be extracted from the shared edge write-shape logs.
Host headroom stayed high (`cpu_idle_avg=81.00%`, `io_wait_avg=0.56%`,
`disk_idle_avg=91.62%`). The next action is to aggregate the new
`pcg_unwind_merge_chain_profile` lines from the remote run artifact; SSH timed
out immediately after the run completed, so the profile split is pending rather
than guessed.
