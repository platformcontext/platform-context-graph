...
| this PR slice | Make `PCG_LARGE_GEN_MAX_CONCURRENT` default to `4` for `local_authoritative` only | Remove source-local large-generation semaphore wait while preserving reducer readiness gates and non-local defaults | Same hot20 corpus and same PCG/NornicDB heads moved `307s` to `240s`; all `20` projector and `160` reducer items succeeded; CPU idle fell `69.77%` to `62.91%`, IO wait stayed low at `2.06%`, disk idle stayed high at `82.07%`; large-generation semaphore acquisitions were immediate | Real 20-repo wall-clock and scheduling/admission win; promoted only for local-authoritative runs, not a reducer worker-count increase |
