...
- fix the separate dead-code post-policy underfill follow-up: the graph query now correctly excludes non-code labels before `LIMIT`, but `results=[]` with `truncated=true` proves policy roots can still consume the bounded candidate window before displayable candidates are found
