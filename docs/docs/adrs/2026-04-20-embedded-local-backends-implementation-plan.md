...
- rerun the same self-repo dogfood with the patched NornicDB binary plus semantic row-property writes, then compare total wall time, reducer completion, and graph-backed query correctness against the earlier canonical-only success
