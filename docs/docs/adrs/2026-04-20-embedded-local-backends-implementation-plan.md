...
- the first rerun with that batched semantic writer still timed out after `18.140664853s`, which proves the remaining semantic issue is statement width rather than the stale scalar writer alone. Current branch now adds `PCG_NORNICDB_SEMANTIC_ENTITY_LABEL_BATCH_SIZES=Function=15,Variable=10,...` and includes the semantic label/row count in bounded-write timeout errors so the next run identifies the exact hot statement instead of failing with a blind `context deadline exceeded`.

Current tuning plan:
- keep the safer `Function=15,Struct=50,Variable=10` row-cap baseline in code while we gather better evidence
- rerun the same self-repo dogfood with the patched NornicDB binary plus the semantic label caps, then compare total wall time, reducer completion, and graph-backed query correctness against the earlier canonical-only success
- if semantic materialization still times out, capture NornicDB CPU/heap profiles during that reducer domain before changing PCG statement shape again
