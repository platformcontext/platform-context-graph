...
- rerun the same self-repo dogfood with the Go semantic write-set reduction; expected proof is that `semantic_entity_materialization` drops materially from the prior `131.247187s` wall while semantic readiness still publishes for enriched Go callables and non-Go semantic labels
