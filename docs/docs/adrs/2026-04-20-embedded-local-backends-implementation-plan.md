...
**Status:** In Progress (local host, local-authoritative path, and backend conformance shipped; production comparison, install trust, OCI plugins, and any Neo4j deprecation path remain)
...
lifecycle, explicit-source NornicDB installer path, progress view, and the
docs/runtime split between local binaries, default NornicDB Compose, and Neo4j
compatibility Compose. PCG currently tracks latest NornicDB `main` through
explicit `--from` installs or `PCG_NORNICDB_BINARY`; the embedded release
manifest intentionally has no accepted assets.

**Remaining work:** keep latest-main NornicDB validation current, decide whether
Neo4j remains a production-promoted backend or a compatibility fallback, finish
the install trust policy and broader host envelope, complete OCI/plugin
contract work, and define any explicit Neo4j deprecation path before closing
this plan. Graph-analysis work has shipped useful local-authoritative read
paths, but broader hardening and truth-class promotion remain tracked work.
...
| Shipped / in progress | 5, 5b | Backend conformance has a matrix, DB-free Go harness, live Compose-backed adapter check, and explicit NornicDB profile gates for `local_authoritative`, `local_full_stack`, and `production`. The latest-main production-profile proof drained the full corpus in `878s` and passed API/MCP evidence checks. A stopped Neo4j comparison snapshot shows NornicDB finished before Neo4j reached source-local completion; the remaining decision is whether Neo4j receives a parity optimization pass or stays compatibility-only. |
| Blocked / not started | 6, 7 | OCI collector plugins and any Neo4j deprecation path are not complete. Chunk 7 must not start until production promotion has a comparison baseline and a migration/deprecation plan exists. |
...
| 3.5 | NornicDB laptop sidecar + `local_authoritative` profile | In progress | `0e4d8a5f`, current branch profile/backend and runtime-gating slices, `da35d729`, current branch authoritative sidecar lifecycle + shared Bolt-driver path + graph-aware reclaim, manual smoke with `/tmp/nornicdb-headless` showing healthy owner + clean Ctrl-C shutdown, current branch binary verification + random workspace credentials, `575ca864` opt-in syntax/workaround gates, `5f5a781e` schema-dialect router + `TestNornicDBSchemaAdapterVerification` pass, current branch explicit-source `pcg install nornicdb --from <source>` installer with managed `${PCG_HOME}/bin/nornicdb-headless` discovery, local binaries, local archives, macOS packages, URLs, SHA-256 checks, and remote download cancellation/timeout handling; current branch intentionally leaves the embedded release manifest empty while PCG tracks latest NornicDB `main`, so bare no-argument install fails with explicit latest-main `--from` guidance instead of using the old forked asset; current branch `pcg graph logs` workspace log reader, current branch owner-aware `pcg graph stop`, current branch foreground `pcg graph start` local-host shortcut, current branch stopped-owner `pcg graph upgrade --from <source>`, 2026-04-22 smoke proving install → start → status running → logs → stop → status stopped, current branch NornicDB grouped-write capability router with `PCG_NORNICDB_CANONICAL_GROUPED_WRITES=true` reserved for conformance, current branch `TestLocalAuthoritativeStartupEnvelope` with cold start `9.045253708s` and warm restart `490.996625ms`, current branch `pcg watch` / `pcg graph start` live local progress panel from the shared status store, 2026-05-03 full-corpus latest-main proof after upstream handoff for `orneryd/NornicDB#136`, current branch production-profile proof `pcg-full-pr138-a2c630af-b68b4ef-20260504T120630Z`, and current branch `TestServiceRunAcksSuccessfulProjectionAfterShutdownCancel` proving completed projector work uses a bounded shutdown ack context instead of ending foreground `pcg graph start` with the `ack projector work: begin: context canceled` tail | install trust policy, broader host coverage, broader query/memory perf envelope |
...
| 5b | NornicDB conformance across profiles | In progress | current branch adds `profile_matrix` gates to `specs/backend-conformance.v1.yaml`, validates those gates in `TestBackendConformanceMatrixDefinesNornicDBProfileGates`, records `local_authoritative` startup/query/full-corpus API/MCP evidence, marks `local_full_stack` against the NornicDB Compose E2E matrix with `PCG_QUERY_PROFILE=local_full_stack`, and records production-profile proof `pcg-full-pr138-a2c630af-b68b4ef-20260504T120630Z`: latest NornicDB `main` `b68b4ef`, `895` corpus Git configs, `896` API repositories, `8458/8458` queue rows succeeded, `0` pending/in-flight/retrying/failed/dead-letter, terminal in `878s`, API/MCP health green, and relationship-evidence drilldown successful on both surfaces. The stopped Neo4j comparison snapshot `pcg-neo4j-baseline-pr138-a2c630af-20260504T123656Z` had `553/896` source-local successes at `1946s` with `0` retrying/failed/dead-letter rows, so it is SLO evidence for NornicDB but not a terminal Neo4j baseline. | decide whether Neo4j gets a parity optimization pass or remains compatibility-only; rerun a terminal Neo4j baseline only if Neo4j stays production-promoted |
...
- 2026-05-04 production-profile proof
  `pcg-full-pr138-a2c630af-b68b4ef-20260504T120630Z` rebuilt PCG
...
- the latest-main NornicDB full-corpus lane is promotion-positive for Chunk 5b:
  PCG `a2c630af` with NornicDB `main` `b68b4ef` drained `8458/8458` work
...
- decide the Neo4j support posture before closing Chunk 5b. If Neo4j remains a
  production-promoted alternative for companies that already pay for and
  operate Neo4j, give it a focused adapter optimization pass and rerun a
  terminal baseline. If it remains compatibility-only, document that explicitly
  so users understand why NornicDB carries the fast path.
...
- keep rebuilding dogfood NornicDB headless binaries directly from upstream `main` for merged fixes such as PRs `#115`, `#116`, and `#118`; use explicit `PCG_NORNICDB_BINARY` / installer `--from` binaries for active compatibility lanes
- keep using explicit latest-main `PCG_NORNICDB_BINARY` / installer `--from` binaries until a future release or accepted-build policy replaces that flow
