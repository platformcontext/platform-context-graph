...
- rerun the same self-repo dogfood with the patched NornicDB binary plus the batched semantic writer, then compare total wall time, reducer completion, and graph-backed query correctness against the earlier canonical-only success
