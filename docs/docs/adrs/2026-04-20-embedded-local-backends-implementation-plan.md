...
**Status:** In Progress (local host and local-authoritative path shipped; backend conformance harness started; latest-main and profile-matrix gates remain)
...
**Remaining work:** keep latest-main NornicDB validation current, wire live
backend conformance runs where they belong, finish profile matrix runs,
OCI/plugin contract work, and any explicit Neo4j deprecation path before
closing this plan. Graph-analysis work has shipped useful local-authoritative
read paths, but broader hardening and truth-class promotion remain tracked
work.
...
| Shipped with open promotion gates | 3.5, 4 | `local_authoritative` NornicDB lifecycle, installer/upgrade/log/stop commands, backend-routed graph queries, query selector normalization, graph-analysis compose coverage, and full-corpus latest-main evidence are present. They are not a completed backend promotion until latest-main validation, install trust policy, and conformance/profile-matrix gates pass. |
| Started | 5 | Backend conformance now has a matrix and DB-free Go harness. Live adapter/profile runs still need to be wired before this chunk is closed. |
| Blocked / not started | 5b, 6, 7 | NornicDB profile matrix, OCI collector plugins, and any Neo4j deprecation path are not complete. Chunk 7 must not start until 5b passes and a migration/deprecation plan exists. |
...
| 5 | Backend conformance suite | In progress | current branch adds `specs/backend-conformance.v1.yaml`, documents the backend matrix, and adds `go/internal/backendconformance` with matrix validation plus reusable `GraphQuery` and Cypher write corpora; current branch `TestNeo4jReaderSatisfiesBackendConformanceGraphQuery`, `TestReducerNeo4jExecutorRunsBackendConformanceWriteCorpus`, and `TestNornicDBGroupedConformanceExecutorRunsBackendConformanceWriteCorpus` prove the current read and reducer write adapter surfaces consume the harness without live databases | live adapter conformance runs in the appropriate integration/Compose/remote lanes; profile-matrix proof remains Chunk 5b |
...
- finish the combined `#119 + #120` full-corpus timing run, then keep using explicit latest-main `PCG_NORNICDB_BINARY` / installer `--from` binaries until a future release or accepted-build policy replaces that flow.
...
- only promote the latest-main lane after the NornicDB fixes are merged and conformance/profile evidence agrees; until then, use `PCG_NORNICDB_BATCHED_ENTITY_CONTAINMENT=true` only for focused evaluation
