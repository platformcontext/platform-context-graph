...
- let the `dogfood-mapmerge-unique-*` remote run complete, then compare total wall time, final `Function` / `Variable` summaries, and query correctness against the previous file-scoped and map-merge-only runs
- rebuild the pprof-enabled NornicDB branch (`--enable-pprof` / `NORNICDB_ENABLE_PPROF=true`) and rerun the same self-repo dogfood while collecting CPU and heap profiles during `Function` / `Variable`, so we can separate Badger writes, uniqueness-index maintenance, Cypher execution, and Bolt/transaction overhead before changing another default
