...
- The latest remote timing run `/tmp/pcg-perf-20260426T112652Z-relset-hotpath` is the current evidence lane until `#120` merges. It uses PCG `9e2bc7e3` and NornicDB `3239983`, passed self-repo in `20s`, passed the focused Crossplane repo in `5s`, crossed the old `90s` full-corpus wall, and at `16m44s` was still making forward progress with projector `succeeded=247`, reducer `succeeded=1812`, and `dead_letter=0`. This keeps the plan focused on preserving cross-repo concurrency while fixing true query-shape and database hot-path gaps; do not add a global NornicDB graph-write throttle unless a later run proves contention remains after those hot paths are release-backed.
...
- keep rebuilding dogfood NornicDB headless binaries directly from latest upstream `main` for merged PRs `#115`, `#116`, `#118`, and `#119`; use the PR `#120` branch binary explicitly via `PCG_NORNICDB_BINARY` / installer `--from` for the active full-corpus compatibility lane until it merges
