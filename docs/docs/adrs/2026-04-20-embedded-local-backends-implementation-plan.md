...
- current branch now keeps the split node-upsert / containment-edge shape for backends that support node-only batched `MERGE`, but routes the pinned NornicDB release through the proven file-scoped combined shape: each statement matches the `File` anchor with `$file_path`, unwinds entity rows for that file, upserts nodes, and attaches `CONTAINS` in the same statement. The opt-in syntax gate caught that the current release-backed NornicDB binary collapses the standalone node-only batch shape, while the combined shape preserves row-bound entity identity. A new NornicDB hot-path regression in `/Users/allen/os-repos/NornicDB-pcg-map-merge-hotpath` proves the desired faster shape needs row-safe `SET += row.props` support inside the generalized `UNWIND/MERGE` batch path; PCG therefore keeps `PCG_NORNICDB_BATCHED_ENTITY_CONTAINMENT=true` as an explicit patched-binary evaluation switch until that NornicDB fix is released and pinned.

Current tuning plan:
- keep the safer `Function=15,Struct=50,Variable=10` row-cap baseline in code while we gather better evidence
- rerun self-repo dogfood with graph schema bootstrap enabled and the patched NornicDB map-merge hot-path binary, first with the safe file-scoped combined entity write and then with `PCG_NORNICDB_BATCHED_ENTITY_CONTAINMENT=true`, using rolling `nornicdb entity label summary` totals to compare correctness and cumulative entity-write cost before/after
