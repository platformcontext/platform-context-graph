...
- upstream handoff now includes a third clean PR, `https://github.com/orneryd/NornicDB/pull/118`, for unique-constraint-backed `MERGE` lookup. Copilot review comments are resolved in the fork branches for both `#116` and `#118`: row counts now reflect required `MATCH` filtering, unsupported `OPTIONAL MATCH` mutations no longer fall through to the read-only UNWIND path, and non-comparable unique lookup values become safe misses instead of panics.
...
- rerun the same self-repo dogfood after the SQL-relationship path is fixed, then compare total wall time, reducer completion, and graph-backed query correctness against the completed stacked-binary run
