...
- treat `PCG_REDUCER_WORKERS=1` as a stability baseline, not the final performance answer: the next benchmark should compare bounded reducer worker counts against the corrected drain gate and NornicDB transient-conflict telemetry before deciding whether the default can safely move above one worker
