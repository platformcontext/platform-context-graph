...
- a follow-up diagnostic run with `PCG_CANONICAL_WRITE_TIMEOUT=30s` moved the full-corpus wall to `1694s` and proved the timeout knob is effective, but it exposed the next real write shape instead of finishing: `semantic_entity_materialization` timed out on `Annotation rows=500`. Current branch now adds `Annotation=100` to the NornicDB semantic label row-cap defaults so huge annotation batches split before they monopolize the graph writer.

Current tuning plan:
- keep the safer `Annotation=100,Function=15,Struct=50,Variable=10,Module=10,ImplBlock=10` row-cap baseline in code while we gather better evidence
...
- keep `PCG_CANONICAL_WRITE_TIMEOUT=15s` for evidence runs so statement-shape regressions fail loudly; production-oriented validation can raise it, but timeout errors must continue to name the env var and statement summary
- use the PR `#119` `pcg-sql-edge-hotpath` branch binary for dogfood until the owner merges it; after merge, rerun the SQL-edge lane from upstream `main` rather than the temporary fork branch
