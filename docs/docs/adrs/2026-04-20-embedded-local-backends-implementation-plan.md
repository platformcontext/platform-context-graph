...
- inspection of PCG's Go-owned graph schema then found the exact asymmetry behind that attribution: `Function` and `Variable` had single-property `uid` uniqueness constraints, but the slow module-like semantic labels did not. The current branch now adds `uid` uniqueness constraints for `Module`, `ImplBlock`, `Protocol`, and `ProtocolImplementation` while preserving the separate `Module.name` index used by canonical import-graph writes. `TestSchemaStatementsContainsUIDConstraints` was tightened from substring matching to exact statement matching after it falsely passed `module_uid_unique` via `terraform_module_uid_unique`, and `TestEnsureSchemaWithBackendExecutesNornicDBStatements` now proves the NornicDB schema bootstrap executes those constraints.
...
- stop adding semantic label caps blindly: attribution now proves the remaining semantic wall is `Module` / `ImplBlock` / `Protocol`, not high-cardinality `Function`. The next clean remote self-repo run should first prove whether the missing `uid` constraints collapse those labels onto NornicDB's unique-lookup hot path before changing any more PCG batch defaults.
