...
- the next full-corpus timing pass used the narrowed Function cap and failed later at `1859s` on `Annotation rows=100` reaching the same `30s` budget. Current branch narrows only the NornicDB semantic `Annotation` cap to `50` and keeps the timeout as an explicit operator knob rather than raising it again.

Current tuning plan:
- keep the safer `Annotation=50,Function=10,Struct=50,Variable=10,Module=10,ImplBlock=10` row-cap baseline in code while we gather better evidence
