...
- the follow-up multi-repo rerun with the `K8sResource` row cap moved the `helm-charts` scope through canonical projection instead of timing out: `K8sResource` completed `275` rows across `193` statements in `131.135765046s` with `max_statement_rows=5` and max grouped execution `11.914367608s`, under the `15s` deadline. The next and current blocker shifted to the huge PHP API repo (`216,610` facts), where the generic `files` phase grouped `15` 500-row file-upsert statements and hit the same `15s` deadline. Current branch now tags file statements as `phase=files`, emits file-row summaries, and adds a NornicDB-only `PCG_NORNICDB_FILE_PHASE_GROUP_STATEMENTS` cap (`5` default) so large file inventories can narrow only file-upsert grouped transactions without lowering every non-entity phase.
