...
- stop adding semantic label caps blindly: attribution now proves the remaining semantic wall is `Module` / `ImplBlock` / `Protocol`, not high-cardinality `Function`. The next slice should inspect the exact NornicDB execution path for those semantic row-property `MERGE` statements and decide whether the fix belongs in NornicDB's hot-path planner/cache/index behavior or a narrow documented PCG backend dialect seam.
