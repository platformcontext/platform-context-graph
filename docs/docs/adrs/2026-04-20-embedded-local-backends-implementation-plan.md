...
Current tuning plan:
- keep the safer `Function=15,Struct=50,Variable=10,Module=10` row-cap baseline in code while we gather better evidence
- rerun the same self-repo dogfood with the Go semantic write-set reduction plus `Module=10`; expected proof is that `semantic_entity_materialization` drops materially from the prior `131.247187s` wall while semantic readiness still publishes for enriched Go callables and non-Go semantic labels
