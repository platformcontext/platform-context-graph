...
binaries, default NornicDB Compose, and Neo4j compatibility Compose. PCG
...
`PCG_NORNICDB_BINARY`. Deployment binaries remain plain service binaries that
...
- manual local-authoritative lifecycle smoke:
  ...
  printed the NornicDB process-mode log; `./go/bin/pcg graph stop --workspace-root <repo>`
