...
- the current full-corpus lane `/tmp/pcg-perf-20260426T124838Z-119-120-full` now uses PCG `323b6b53` and NornicDB `86e78f1`. It passed the self-repo warmup in `20s`, passed focused Crossplane in `5s`, and at the early 896-repo checkpoint was progressing with projector `succeeded=63`, reducer `succeeded=455`, `dead_letter=0`, and no overdue claims. This is the evidence lane that decides whether any remaining bottleneck is PCG flow shape, NornicDB hot-path coverage, or expected first-generation corpus throughput.
