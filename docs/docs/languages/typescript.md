...
| Declaration merging | `declaration-merging` | partial | `functions`, `classes`, `interfaces`, `modules`, `enums` | `name, declaration_merge_group, declaration_merge_count, declaration_merge_kinds` | `content:Entity.metadata.declaration_merge_* + semantic_summary + semantic_profile + story` | `go/internal/parser/engine_typescript_advanced_semantics_test.go::TestDefaultEngineParsePathTypeScriptCapturesDeclarationMerging`, `go/internal/query/entity_metadata_typescript_semantics_test.go::TestEnrichEntityResultsWithContentMetadataTypeScriptDeclarationMerging`, `go/internal/query/repository_story_typescript_semantics_test.go::TestBuildRepositorySemanticOverviewCountsTypeScriptAdvancedSignals` | Compose-backed fixture verification | Same-file TypeScript declaration merging between supported declarations now survives the Go parser and is surfaced through semantic summaries, profiles, and stories. First-class graph persistence remains partial. |
