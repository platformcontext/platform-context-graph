...
- Notes:
  ...
  - Type aliases are queryable through the Go content-backed APIs, and
    graph-backed `language-query`, `code/search`, `entities/resolve`, and
    entity-context results now enrich matching rows with decorator and generic
    metadata. Dedicated graph/story/context surfacing remains partial.
