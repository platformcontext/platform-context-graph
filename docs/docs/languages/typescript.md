...
- Notes:
  ...
  - Type aliases now persist as first-class `TypeAlias` graph nodes through
    ...
    decorator and generic metadata, and `language-query`, `code/search`, plus
    entity-context now emit both semantic summaries and a structured
    `semantic_profile` for matching graph-backed entities. Entity-context now
    also emits a first-class `story`, and repository stories now expose a
    semantic overview derived from those same entities.
