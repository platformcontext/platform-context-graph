...
- Notes:
  ...
  - JSDoc and method-kind semantics already survive parse and content
    ...
    entity-context now emits a first-class `story`, and repository stories
    now expose a semantic overview derived from those entities. Dedicated
    graph-first promotion is still partial.
