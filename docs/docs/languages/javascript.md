...
- Notes:
  ...
  - JSDoc and method-kind semantics already survive parse and content
    ...
    and `story` when the graph row already has it. `dead-code`,
    `code/relationships`, `code/complexity`, `entities/resolve`, and the
    remaining non-JavaScript entity-context/story paths still surface the
    matching metadata through content enrichment when the graph does not
