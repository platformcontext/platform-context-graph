...
- Notes:
  ...
  - JSDoc and method-kind semantics already survive parse and content
    materialization, and graph-backed `language-query` plus `code/search`
    results now surface the matching metadata. Broader graph/story/context
    surfacing is still partial.
