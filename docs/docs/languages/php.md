...
| Static method calls | `static-method-calls` | partial | `function_calls` | `name, line_number` | `relationship:CALLS` | `go/internal/parser/php_language_test.go::TestDefaultEngineParsePathPHPEmitsVariableAndCallMetadata` | Compose-backed fixture verification | Static call syntax is now normalized with inferred receiver metadata, and focused parser tests prove both direct class receivers and `self` resolution. The comprehensive fixture repo still does not exercise a dedicated static-call graph edge end to end. |
