...
- Notes:
  ...
  - Python function semantics for decorators, async, and `semantic_kind=lambda`
    ...
    directly by `code/language-query`; content-backed surfaces still use the
    shared enrichment path where graph data is absent.
  - Type annotations are queryable through the Go content-backed APIs, the
    ...
    `story` output. Python metaclass ownership now persists as a graph-backed
