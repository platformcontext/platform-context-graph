...
- Notes:
  - Framework evidence for FastAPI and Flask is carried by the Go parser and
    indexing path.
  - Type annotations are queryable through the Go content-backed APIs, and the
    normal entity resolve/context surfaces now fall back to content-backed
    entities. Graph-backed `language-query`, `code/search`, `dead-code`,
