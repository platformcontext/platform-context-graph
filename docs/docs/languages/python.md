...
- Notes:
  ...
  - Type annotations are queryable through the Go content-backed APIs, the
    ...
    content-backed entity names as well as source text. Graph-backed `language-query`, `code/search`, `dead-code`,
    `code/relationships`, `code/complexity`, `entities/resolve`, and
    entity-context results now enrich matching rows with decorator and async
    metadata. `language-query`, `code/search`, and entity-context also emit
    semantic summaries for Python decorator, async, and type-annotation
    semantics. Fully first-class graph/story/context surfacing remains
    partial.
