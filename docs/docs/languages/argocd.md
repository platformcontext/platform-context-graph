...
| Sync policy | `sync-policy` | partial | `argocd_applications` | `name, line_number` | `none:not_persisted` | `go/internal/parser/engine_yaml_semantics_test.go::TestDefaultEngineParsePathYAMLArgoCDApplication` | Compose-backed fixture verification | The native YAML parser does not currently normalize `syncPolicy` into the Argo CD payload. |
