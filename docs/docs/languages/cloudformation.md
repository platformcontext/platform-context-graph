...
| JSON templates | `json-templates` | partial | `file_format` | `name, line_number` | `none:not_persisted` | `go/internal/parser/engine_infra_test.go::TestDefaultEngineParsePathJSONCloudFormation` | Compose-backed fixture verification | JSON-formatted templates are detected at parse time, but the comprehensive fixture and end-to-end indexing coverage are still YAML-first. |
