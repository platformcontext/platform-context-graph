...
- Notes:
  ...
  - TSX type aliases now persist as first-class `TypeAlias` graph nodes,
    ...
    now includes `React.FC<...>`-style assertions and `lazy(...)` wrappers, and
...
- TSX-specific type narrowing patterns such as `as ComponentType<...>` now have
  ...
  including qualified component types such as `React.ComponentType<...>` and
  broader wrapper forms such as `React.FC<...>` and `lazy(...)`
