...
| Historical patch-link heuristic | `patch-link-heuristic` | supported | content-backed relationships | `patch_targets` | `relationship:PATCHES` | `go/internal/query/content_relationships_kustomize_test.go::TestBuildContentRelationshipSetKustomizeOverlayPatchesTargetResource` | `go/internal/query/entity_content_iac_fallback_test.go::TestGetEntityContextFallsBackToKustomizeOverlayContentEntity` | Preserves the historical overlay-to-target patch link on the Go-owned query path. |
...
- Go now surfaces patch targets and the historical patch-link heuristic on the normal query path, but the typed Kustomize evidence for resources vs Helm vs images still needs fuller final graph/query promotion
