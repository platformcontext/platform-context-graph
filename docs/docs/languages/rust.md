...
| Impl blocks | `impl-blocks` | partial | `impl_blocks` | `name, line_number, kind` | `content:ImplBlock entity + code/language-query` | `go/internal/parser/engine_systems_test.go::TestDefaultEngineParsePathRustImplBlocks`, `go/internal/query/language_queries_test.go::TestHandleLanguageQuery_ContentBackedEntityTypes` | Compose-backed fixture verification | The Go parser now emits dedicated impl block records, the content layer persists them as `ImplBlock` entities, and the normal Go `code/language-query` surface can return them. Function `impl_context` remains available, but graph/story surfacing still does not expose explicit implementation edges end to end. |
