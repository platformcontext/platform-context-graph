...
| Annotations (applied) | `annotations-applied` | partial | `annotations` | `name, line_number, kind, target_kind` | `content:Annotation entity + code/language-query` | `go/internal/parser/engine_managed_oo_test.go::TestDefaultEngineParsePathJavaAnnotationUsageKinds`, `go/internal/query/language_queries_test.go::TestHandleLanguageQuery_ContentBackedEntityTypes` | Compose-backed fixture verification | Applied annotations are now tagged with their usage kind and nearest declaration target kind in the parser contract, and the normal Go `code/language-query` surface can return persisted `Annotation` content entities. Graph/story promotion is still partial. |
