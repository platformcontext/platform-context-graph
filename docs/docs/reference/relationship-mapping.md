...
| Jenkins / Groovy | Jenkinsfile metadata, stage and command hints, reusable pipeline metadata | Explicit shared-library refs, including `library(...)` step forms, and explicit GitHub repository URLs now emit canonical repo evidence, while the Go query path also surfaces controller-artifact summaries for `Jenkinsfile` inputs, entry points, shared libraries, pipeline calls, and Ansible playbook hints; broader controller-driven promotion remains active parity work on this branch |
| Ansible | playbooks, inventories, `group_vars`, `host_vars`, targeted roles, task entrypoints | The Go path now classifies inventories, vars, playbooks, role roots, and task entrypoints explicitly, playbooks emit truthful role-reference evidence, controller stories can now surface Ansible playbook hints coming from Jenkins/Groovy control-plane artifacts, and the query path surfaces those config assets; broader controller-driven deployment meaning and stronger verb promotion remain active parity work on this branch |
