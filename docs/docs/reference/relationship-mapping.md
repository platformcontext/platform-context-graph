...
| Ansible | playbooks, inventories, `group_vars`, `host_vars`, targeted roles, task entrypoints | The Go path now classifies inventories, vars, playbooks, role roots, and task entrypoints explicitly, but only playbook content that truthfully resolves against the repo catalog emits canonical role-reference evidence. Inventories, vars, and controller playbook hints stay read-side unless they resolve to repo evidence; the query path still surfaces those config assets for context. Broader controller-driven deployment meaning and stronger verb promotion remain active parity work on this branch |
...
- broader Ansible automation evidence beyond artifact typing, role-reference evidence, and query-visible config assets
