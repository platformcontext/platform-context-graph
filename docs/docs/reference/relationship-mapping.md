...
| Ansible | playbooks, inventories, `group_vars`, `host_vars`, targeted roles, task entrypoints | The Go path now classifies inventories, vars, playbooks, role roots, and task entrypoints explicitly, and playbook content that truthfully resolves against the repo catalog emits canonical role-reference evidence. Jenkins-driven controller artifacts plus repository deployment/story shaping now also carry adjacent Ansible inventories, vars, and task entrypoints instead of dropping them on the floor. Broader controller-driven deployment meaning and any stronger verb promotion beyond those query-visible automation summaries remain active parity work on this branch |
| Docker / Compose | Dockerfile build/runtime hints, Compose services, image wiring, env/config links, dependency hints | Docker Compose build contexts, including shorthand `build: ../repo`, and image refs now emit canonical deploy-source evidence, explicit `depends_on` service names now emit canonical dependency evidence when they resolve truthfully through the repo catalog, Dockerfile source labels such as `org.opencontainers.image.source` now emit truthful canonical `DEPLOYS_FROM` evidence, and the Go read side now surfaces runtime artifact summaries for both Compose services and Dockerfile stages. The relationship-platform compose proof now verifies those Compose `build` and `depends_on` signals end to end against the live Go runtime. Plain Docker base images and `COPY --from` stage aliases remain bounded read-side signals rather than fake repo edges. Broader controller/runtime promotion remains active parity work on this branch |
...
- broader Ansible automation evidence beyond artifact typing, role-reference evidence, and the now query-visible controller summaries for inventories, vars, and task entrypoints
