...
| Variable | Default | Read By | Purpose | Tune When |
| --- | --- | --- | --- | --- |
| `PCG_PROJECTOR_WORKERS` | Default: `min(NumCPU, 8)`; NornicDB local-authoritative: `1` | ingester projector | Source-local projector worker count. | Raise when source-local projection is CPU-bound and graph backend can absorb writes; keep NornicDB local-authoritative serialized until backend write concurrency is proven safe. |
...
| `PCG_REDUCER_WORKERS` | Neo4j: `min(NumCPU, 4)`; NornicDB: `min(NumCPU, 8)` | reducer | Reducer intent worker count. | Lower only when conflict-domain fencing still shows graph write conflicts or backend saturation. Raise explicitly only after measuring queue tail, CPU, disk, and graph-write timeout rates. |
| `PCG_REDUCER_BATCH_CLAIM_SIZE` | Neo4j: `workers*4` capped `4..64`; NornicDB: `workers` | reducer | Number of reducer intents claimed per poll. | Keep near worker count for slow graph backends so every claimed item can start heartbeat-protected execution promptly. |
...
| `PCG_REDUCER_CLAIM_DOMAIN` | unset | reducer | Restricts the main reducer claim loop to one reducer domain such as `sql_relationship_materialization` or `deployment_mapping`. Shared projection side runners still use their own lease controls. | Use for domain-lane diagnostics or controlled split-reducer experiments after proving graph truth on a focused corpus. Do not use it to hide stuck work; run an all-domain reducer or matching domain lanes for every required domain. |
