...
| Kustomize parity | `partial` | parser payload parity is met and now exceeds Python with first-class sorted `bases`, inline patch-target extraction, and typed Kustomize evidence for resources vs Helm vs images. The historical patch-link heuristic now survives on the Go-owned query path; the remaining gap is fuller graph/query promotion of the typed deploy-source signals | Kustomize parser payloads plus the historical typed evidence/link behavior survive parse, relationships, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
