...
| Canonical code-call materialization | `partial` | the collector and reducer now own canonical `CALLS` edge writes for SCIP-backed parser facts plus unambiguous same-file generic `function_calls`, including repo-scoped retraction and rewrite during normal Go ingestion. Primary non-SCIP families now also preserve richer generic call metadata (`full_name`, and JSX/function kind where available), but cross-file non-SCIP and long-tail language call-edge parity is still incomplete | required language families persist canonical call edges through parser, collector, reducer, and query proof without Python fallback | parser tests, reducer/collector tests, query proof | pair with parser-family waves |
