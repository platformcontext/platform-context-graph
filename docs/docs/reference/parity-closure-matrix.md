...
| SQL/dbt lineage | `partial` | dbt compiled lineage still loses truly opaque templated expressions and opaque macro bodies; templated wrappers around those opaque forms remain a deliberate non-goal, while non-templated opaque wrappers were historically inconsistent in Python rather than one clean unresolved class. The Go parser now preserves lineage through safe wrappers, lineage-preserving macro wrappers that still expose real source refs, `concat_ws`, `md5`, fixture-backed wildcard-plus-`coalesce(...)` lineage, top-level Jinja wrappers around those same safe forms, and checked-in materialization/query proof for `AnalyticsModel` plus `DataAsset` content entities. The remaining gap is closing the last unresolved expression classes and broadening real-repo proof | compiled dbt lineage survives parse, materialization, and query proof | parser tests, real-repo proof, compose proof | dedicated medium/large wave |
...
| Wave 1 | SQL/dbt lineage | largest semantic gap and mostly isolated; now narrowed to truly opaque templated expressions, opaque macro bodies, the remaining richer derived-expression families outside the safe wrapper set, and broader real-repo proof after macro-wrapper, `concat_ws`/`md5`, wildcard-plus-`coalesce(...)`, and template-wrapper lineage recovery |
