...
| ArgoCD parity | `partial` | parser payload parity is met and now exceeds Python with sync-policy normalization plus explicit ApplicationSet generator/template source split, and Go now emits discovery, deploy-source, and destination-platform evidence for ArgoCD Applications/ApplicationSets. The remaining gap is final graph/query promotion of the full chain. Helm-specific source parameters and custom health checks were also Python limitations, so they are not the primary parity blocker | ArgoCD parser payloads plus the historical Application/ApplicationSet evidence chain survive parse, relationships, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
