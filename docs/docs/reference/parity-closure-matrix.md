...
| Kubernetes parity | `partial` | parser payload parity is met and now exceeds Python with `qualified_name`, but the old Python `Service -> Deployment` `SELECTS` heuristic has not yet been ported to Go. Real `selector` / `matchLabels` resolution and CRD schema awareness were historical Python limitations too, so they are not parity blockers by themselves | Kubernetes parser payloads plus the historical `SELECTS` heuristic survive parse, evidence, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
