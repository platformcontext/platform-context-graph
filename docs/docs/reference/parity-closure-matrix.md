...
| TSX graph parity | `partial` | JSX tag usage is captured, and type aliases plus component semantics are queryable through Go content-backed APIs with content-backed entities now participating in normal entity resolve/context, but graph/reference-edge and higher-level surfacing remain partial | JSX component semantics become queryable and proven end to end | parser tests, graph tests, API/MCP proof | shared JS-family wave |
