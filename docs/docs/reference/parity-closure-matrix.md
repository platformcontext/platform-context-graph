...
| SQL/dbt lineage | `fail` | dbt compiled lineage still loses truly opaque templated expressions and opaque macro bodies; templated wrappers around those opaque forms remain a deliberate non-goal, while the Go parser now preserves lineage through safe wrappers, lineage-preserving macro wrappers that still expose real source refs, and top-level Jinja wrappers around those same safe forms, and the safe-wrapper transform matrix now has checked-in Go proof | compiled dbt lineage survives parse, materialization, and query proof | parser tests, real-repo proof, compose proof | dedicated medium/large wave |
...
| Workstream | Status | Gap now | Done means | Validation gate | Recommended split |
| --- | --- | --- | --- | --- | --- |
| Terraform parity | `partial` | `terraform {}` block metadata is first-class, but `count`/`for_each` expansion and dynamic block traversal remain incomplete | Terraform structural metadata is first-class where Python exposed it | parser tests, relationship tests, query proof | shared HCL wave |
...
| Kubernetes parity | `partial` | labels and qualified resource normalization are now persisted, but selector/matchLabels relationships and CRD schema awareness remain incomplete | labels, qualified resource identity, and required resource semantics survive graph/query layers | parser tests plus compose-backed proof | shared YAML/IaC wave |
| ArgoCD parity | `partial` | sync policy is normalized and ApplicationSet generator wrappers now flatten to stable source summaries, but Helm-specific source parameters and custom health checks remain incomplete | sync policy and required generator semantics persist and query correctly | parser tests plus compose-backed proof | shared YAML/IaC wave |
...
| Kustomize parity | `pass` | base references persist as a first-class sorted list | base references persist and query correctly | parser tests plus query proof | shared YAML/IaC wave |
