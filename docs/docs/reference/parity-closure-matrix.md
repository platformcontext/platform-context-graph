...
| ArgoCD parity | `partial` | parser payload parity is met and now exceeds Python with sync-policy normalization plus explicit ApplicationSet generator/template source split, Go emits discovery, deploy-source, and destination-platform evidence for ArgoCD Applications/ApplicationSets, the normal Go entity resolve/context fallback now surfaces that chain through semantic summaries plus synthesized `DISCOVERS_CONFIG_IN`, `DEPLOYS_FROM`, and `RUNS_ON` relationships, and the Go-owned `trace_deployment_chain` MCP/API path now returns story-first deployment traces with `deployment_overview`, `deployment_sources`, `cloud_resources`, `deployment_facts`, `deployment_fact_summary`, `controller_driven_paths`, and `delivery_paths`. The remaining gap is deeper canonical K8s/image/backing-resource traversal plus richer repository-story surfacing of the same chain. Helm-specific source parameters and custom health checks were also Python limitations, so they are not the primary parity blocker | ArgoCD parser payloads plus the historical Application/ApplicationSet evidence chain survive parse, relationships, entity-context fallback, and deployment-trace query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
