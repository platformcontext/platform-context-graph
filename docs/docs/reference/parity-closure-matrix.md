...
| JavaScript graph parity | `partial` | docstrings and method-kind metadata are extracted and materialized, and graph-backed `language-query` results now enrich matching rows with that metadata, but broader graph/story/context surfacing remains partial | JS docstrings and method metadata persist and query correctly across parser, content, and normal query surfaces | parser tests plus query proof | shared JS-family wave |
| TypeScript graph parity | `partial` | decorator and generic metadata are preserved, type aliases are already queryable through Go content-backed APIs, and graph-backed `language-query` results now enrich matching rows with metadata, but dedicated graph/story/context surfacing remains partial | those semantics persist and query correctly across parser, content, and normal query surfaces | parser tests, graph tests, API/MCP proof | shared JS-family wave |
...
| Python graph parity | `partial` | decorators and async flags are extracted and materialized, type annotations are already queryable through Go content-backed APIs, and graph-backed `language-query` results now enrich matching rows with metadata, but broader graph/story/context surfacing remains partial | those semantics persist and query correctly across parser, content, and normal query surfaces | parser tests, graph tests, API/MCP proof | parallel medium wave |
...
| Wave 2 | JavaScript, TypeScript, TSX, Python | shared graph/story/context promotion work for already-materialized parser semantics; normal `language-query` metadata enrichment is now in place for graph-backed JS/TS/Python entities, while type aliases, type annotations, and components were already queryable through content-backed surfaces |
