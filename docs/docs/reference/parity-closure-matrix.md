...
| SQL/dbt lineage | `fail` | dbt compiled lineage still loses truly opaque templated expressions and opaque macro expansion, but the Go parser now preserves lineage through safe wrappers, lineage-preserving macro wrappers that still expose real source refs, and top-level Jinja wrappers around those same safe forms, and the safe-wrapper transform matrix now has checked-in Go proof | compiled dbt lineage survives parse, materialization, and query proof | parser tests, real-repo proof, compose proof | dedicated medium/large wave |
...
| Wave 1 | SQL/dbt lineage | largest semantic gap and mostly isolated; now narrowed to truly opaque templated expressions and opaque macro bodies after macro-wrapper and template-wrapper lineage recovery |
