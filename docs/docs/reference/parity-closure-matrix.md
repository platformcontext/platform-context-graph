...
| Kustomize parity | `partial` | parser payload parity is met and now exceeds Python with first-class sorted `bases`, inline patch-target extraction, and typed Kustomize evidence for resources vs Helm vs images. The remaining gap is the old patch-link heuristic plus final graph/query promotion of those typed signals | Kustomize parser payloads plus the historical typed evidence/link behavior survive parse, relationships, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
