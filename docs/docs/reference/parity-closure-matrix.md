...
| ArgoCD parity | `partial` | parser payload parity is met and now exceeds Python with sync-policy normalization, and Go now emits ApplicationSet discovery plus deploy-source evidence facts. The remaining gap is destination-cluster/runtime-platform evidence and final graph/query promotion of the full chain. Helm-specific source parameters and custom health checks were also Python limitations, so they are not the primary parity blocker | ArgoCD parser payloads plus the historical ApplicationSet evidence chain survive parse, relationships, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
