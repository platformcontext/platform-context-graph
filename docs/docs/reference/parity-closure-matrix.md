...
| ArgoCD parity | `partial` | parser payload parity is met and now exceeds Python with sync-policy normalization plus explicit ApplicationSet generator/template source split, Go emits discovery, deploy-source, and destination-platform evidence for ArgoCD Applications/ApplicationSets, and the normal Go entity resolve/context fallback now surfaces that chain through semantic summaries plus synthesized `DISCOVERS_CONFIG_IN`, `DEPLOYS_FROM`, and `RUNS_ON` relationships. The remaining gap is graph-first repository/story/MCP surfacing of the same chain. Helm-specific source parameters and custom health checks were also Python limitations, so they are not the primary parity blocker | ArgoCD parser payloads plus the historical Application/ApplicationSet evidence chain survive parse, relationships, and normal query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
