...
| Rust graph parity | `partial` | impl blocks now persist as `ImplBlock` content entities, are queryable through `code/language-query`, and now surface semantic summaries through normal `entities/resolve` and entity-context fallbacks, but implementation edges are still not first-class on the graph | impl ownership and relationships persist and query correctly | parser tests plus query proof | long-tail wave |
...
| ArgoCD parity | `partial` | parser payload parity is met and now exceeds Python with sync-policy normalization, but the Python-era ApplicationSet discovery/deploy-source/destination-cluster evidence chain is still missing from Go. Helm-specific source parameters and custom health checks were also Python limitations, so they are not the primary parity blocker | ArgoCD parser payloads plus the historical ApplicationSet evidence chain survive parse, relationships, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
...
| Kustomize parity | `partial` | parser payload parity is met and now exceeds Python with first-class sorted `bases`, but the Go relationship/evidence layer still lacks the Python-era typed Kustomize evidence for resources vs Helm vs images and the old patch-link heuristic | Kustomize parser payloads plus the historical typed evidence/link behavior survive parse, relationships, and query proof | parser tests, relationship tests, query proof | shared YAML/IaC wave |
