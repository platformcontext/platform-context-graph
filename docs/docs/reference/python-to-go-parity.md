...
| Kubernetes | Go-owned | partial | YAML resource parsing is complete, and resource identity is now normalized into a stable `namespace/kind/name` string | normalize and persist selector and matchLabels relationships |
| ArgoCD | Go-owned | partial | Applications and ApplicationSets parse in Go; sync policy and ApplicationSet generator wrappers are normalized | normalize and persist Helm-specific source parameters and custom health checks if parity still requires them |
...
| Kustomize | Go-owned | complete | overlays, resources, and base references parse in Go | none; base references now stay first-class as a sorted list |
| Terraform | Go-owned | partial | HCL parser and provider schema support are Go-owned; `terraform {}` block metadata is now a first-class graph surface | close `count`/`for_each` expansion and dynamic block traversal if parity requires them |
...
- Kubernetes: selector and matchLabels relationships
...
- Terraform: `count`/`for_each` expansion and dynamic block traversal
...
- SQL/dbt: compiled lineage maturity remains partial even though the runtime is Go-owned; remaining gaps are unresolved refs, truly opaque templated expressions, opaque macro bodies, wrapped opaque macro bodies, and richer derived expressions
