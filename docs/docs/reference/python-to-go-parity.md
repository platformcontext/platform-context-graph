...
| ArgoCD | Go-owned | partial | Applications and ApplicationSets parse in Go; sync policy and ApplicationSet generator wrappers are normalized; generator discovery inputs are now preserved separately from template deploy sources; discovery, deploy-source, and destination-platform evidence materialize in Go; and the normal entity resolve/context fallback now surfaces that chain through semantic summaries plus synthesized `DISCOVERS_CONFIG_IN`, `DEPLOYS_FROM`, and `RUNS_ON` relationships | promote the same chain through graph-first repository/story/MCP surfaces |
...
- ArgoCD: graph-first repository/story/MCP surfacing for the Application/ApplicationSet evidence chain that already exists in parser, relationships, and entity resolve/context fallback
