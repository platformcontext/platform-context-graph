...
| Elixir | Go-owned | partial | core Elixir parsing is complete, `defprotocol` now materializes as a first-class `Protocol` content entity on the normal Go parser/content/query path, and module/function semantic kinds now surface through normal Go query/context semantic summaries on `Module`, `Protocol`, and `Function` entities | persist guards, protocols, protocol implementations, and module attributes as first-class graph semantics rather than generic metadata |
...
- Elixir: guards, protocols, protocol implementations, and module attributes now appear in semantic summaries on normal query/context responses, and `defprotocol` now materializes as a first-class `Protocol` content entity, but the graph still stores those semantics as generic modules/functions/variables
- ArgoCD: broaden repository-story/controller-overview richness on top of the now-Go-owned `trace_deployment_chain` MCP/API surfacing for the Application/ApplicationSet evidence chain
