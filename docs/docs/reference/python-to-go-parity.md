...
| SQL/dbt lineage and data intelligence | Go-owned | partial | dbt manifest, compiled SQL, and analytics JSON families are in Go; row-level aggregates, simple windows, simple qualified macro wrappers, nested safe wrappers, top-level Jinja wrappers around lineage-safe expressions, cast/date_trunc/concat transforms, and multi-source case/arithmetic lineage now have checked-in Go proof, and `AnalyticsModel` plus `DataAsset` entities now have checked-in content materialization plus normal entity resolve/context proof | resolve unresolved references, truly opaque templated expressions, opaque macro bodies, the remaining historically inconsistent non-templated wrapped-opaque cases, and richer derived expressions that still fall outside the safe wrapper set |
| JavaScript | Go-owned | partial | core JS parsing plus docstring and method-kind metadata extraction/materialization are present, graph-backed query surfaces enrich matching rows with that metadata, and `language-query`, `code/search`, plus entity-context now emit semantic summaries for matching JavaScript entities | promote docstrings and method-kind metadata into fuller first-class graph/story surfaces beyond the current query/context responses |
...
| Kotlin | Go-owned | partial | core Kotlin parsing is complete, secondary constructor metadata now surfaces through normal Go query/context semantic summaries on function entities, and explicit `this.` receiver calls now resolve to canonical same-file `CALLS` edges | persist broader receiver/type inference and constructor semantics as first-class graph/story behavior beyond metadata enrichment |
...
| Rust | Go-owned | partial | core Rust parsing is complete, and impl blocks now persist as `ImplBlock` content entities that the normal Go `code/language-query` surface can return; normal entity resolve/context fallbacks now surface semantic summaries too, and canonical call materialization now honors exact `impl_context`-qualified Rust methods | persist explicit impl-block graph semantics and relationships end to end |
...
| ArgoCD | Go-owned | partial | Applications and ApplicationSets parse in Go; sync policy and ApplicationSet generator wrappers are normalized; ApplicationSet discovery and deploy-source evidence now materialize in Go | finish the destination-cluster/runtime-platform leg of the historical ApplicationSet evidence chain and promote the full chain through graph/query surfaces |
...
| Kustomize | Go-owned | partial | overlays, resources, and base references parse in Go, base references now stay first-class as a sorted list, and typed Kustomize evidence for resources vs Helm vs images now materializes in Go | port the old patch-link heuristic and promote the typed evidence through the final graph/query surfaces |
...
- TypeScript: graph/story/context surfacing for decorators and generics, with graph-backed `language-query`, `code/search`, `dead-code`, `code/relationships`, `code/complexity`, `entities/resolve`, and entity-context metadata enrichment now in place, `language-query` and `code/search` semantic summaries now in place for matching graph-backed entities, plus content-backed entity resolve/context and `code/search` fallback coverage for type aliases
...
- SQL/dbt: compiled lineage maturity remains partial even though the runtime is Go-owned; remaining gaps are unresolved refs, truly opaque templated expressions, opaque macro bodies, the remaining historically inconsistent non-templated wrapped-opaque cases, and richer derived expressions
