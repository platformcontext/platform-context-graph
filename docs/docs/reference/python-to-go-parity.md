...
| Terraform | Go-owned | complete | HCL parser and provider schema support are Go-owned, Go now exceeds the old Python baseline by materializing first-class `terraform {}` block metadata, and resource rows now retain raw `count`/`for_each` expressions | none for Python-era parity; `count`/`for_each` expansion and `dynamic` traversal remain optional net-new follow-on work |
