...
- Python: annotation completeness for rows that still depend on content fallback; parser extraction and the current graph-backed query surfaces for decorators, async functions, generator functions, metaclass ownership, module docstrings, type annotations, and lambda ownership are already in place
