...
| SQL/dbt lineage and data intelligence | Go-owned | partial | dbt manifest, compiled SQL, and analytics JSON families are in Go; row-level aggregates, simple windows, simple qualified macro wrappers, nested safe wrappers, top-level Jinja wrappers around lineage-safe expressions, cast/date_trunc/concat transforms, and multi-source case/arithmetic lineage now have checked-in Go proof | resolve unresolved references, truly opaque templated expressions, opaque macro bodies, the remaining historically inconsistent non-templated wrapped-opaque cases, and richer derived expressions that still fall outside the safe wrapper set |
