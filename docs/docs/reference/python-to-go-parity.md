...
| ArgoCD | Go-owned | partial | Applications and ApplicationSets parse in Go; sync policy and ApplicationSet generator wrappers are normalized; generator discovery inputs are now preserved separately from template deploy sources; and discovery, deploy-source, and destination-platform evidence materialize in Go | promote the full Application/ApplicationSet evidence chain through the final graph/query surfaces |
