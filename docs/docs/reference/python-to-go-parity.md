...
| ArgoCD | Go-owned | partial | Applications and ApplicationSets parse in Go; sync policy and ApplicationSet generator wrappers are normalized | port the historical Python ApplicationSet discovery/deploy-source/destination-cluster evidence chain |
...
| Kustomize | Go-owned | partial | overlays, resources, and base references parse in Go, and base references now stay first-class as a sorted list | port the historical Python typed Kustomize evidence for resources vs Helm vs images and the old patch-link heuristic |
...
- ArgoCD: historical ApplicationSet discovery/deploy-source/destination-cluster evidence chain
- CloudFormation: nested stack references and condition evaluation
- Kustomize: historical typed evidence for resources vs Helm vs images, plus the patch-link heuristic
