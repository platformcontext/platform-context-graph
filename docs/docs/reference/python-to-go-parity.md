...
| JavaScript | Go-owned | partial | core JS parsing plus docstring and method-kind metadata extraction/materialization are present, graph-backed query surfaces enrich matching rows with that metadata, `language-query`, `code/search`, plus entity-context now emit both semantic summaries and a structured `semantic_profile`, entity-context now also emits a first-class `story`, and repository stories now expose semantic coverage through `semantic_overview` | promote docstrings and method-kind metadata into dedicated graph-first modeling beyond the current shared query/story surfaces |
| TypeScript | Go-owned | partial | core TS parsing plus decorator/generic metadata preservation are present; type aliases are queryable through Go content-backed APIs, content-backed entities now participate in normal entity resolve/context, the normal `code/search` fallback can now search content-backed entity names as well as source text, graph-backed `language-query`, `code/search`, `dead-code`, `code/relationships`, `code/complexity`, `entities/resolve`, and entity-context results now enrich matching rows with that metadata, `language-query`, `code/search`, plus entity-context also emit semantic summaries and a structured `semantic_profile`, entity-context now also emits a first-class `story`, and repository stories now expose semantic coverage through `semantic_overview` | promote decorators and generics into dedicated graph-first modeling beyond the shared query/story surfaces |
| TypeScript JSX | Go-owned | partial | core TSX parsing plus JSX tag capture are present; type aliases and component semantics are queryable through Go content-backed APIs, content-backed entities now participate in normal entity resolve/context, the normal `code/search` fallback can now search content-backed entity names as well as source text and emit both semantic summaries and a structured `semantic_profile` for content-backed component entities, normal `code/relationships` now synthesizes JSX component `REFERENCES` edges for content-backed entities, entity-context now also emits a first-class `story`, and repository stories now expose semantic coverage through `semantic_overview` | promote component/reference semantics into full graph-first modeling beyond the content-backed fallback and close the remaining normal-path alias gaps |
| Python language parsing | Go-owned | partial | core Python parsing plus decorator/async extraction are present; type annotations are queryable through Go content-backed APIs, content-backed entities now participate in normal entity resolve/context, the normal `code/search` fallback can now search content-backed entity names as well as source text, graph-backed query surfaces enrich matching rows with that metadata, `language-query`, `code/search`, plus entity-context now emit semantic summaries and a structured `semantic_profile` for Python decorator/async/type-annotation semantics, entity-context now also emits a first-class `story`, and repository stories now expose semantic coverage through `semantic_overview` | promote decorators and async flags into dedicated graph-first modeling beyond the shared query/story surfaces |
...
| Rust | Go-owned | partial | core Rust parsing is complete, impl blocks now persist as `ImplBlock` content entities, the normal Go `code/language-query`, entity-context, and `code/relationships` surfaces now expose impl ownership through exact `impl_context` matching, canonical call materialization now honors exact `impl_context`-qualified Rust methods, and reducer proof covers safe repo-unique cross-file bare-name call materialization for Rust `function_calls` | persist explicit impl-block graph semantics and relationships end to end, then add broader public call-graph/query proof |
| Elixir | Go-owned | partial | core Elixir parsing is complete, `defprotocol` now materializes as a first-class `Protocol` content entity on the normal Go parser/content/query path, module/function semantic kinds now surface through normal Go query/context semantic summaries on `Module`, `Protocol`, and `Function` entities, and the normal Go query path can also resolve `guard`, `protocol_implementation`, and `module_attribute` as semantic aliases over flattened `Function`, `Module`, and `Variable` entities | persist guards, protocols, protocol implementations, and module attributes as first-class graph semantics rather than generic metadata |
...
| ArgoCD | Go-owned | partial | Applications and ApplicationSets parse in Go; sync policy and ApplicationSet generator wrappers are normalized; generator discovery inputs are now preserved separately from template deploy sources; discovery, deploy-source, and destination-platform evidence materialize in Go; the normal entity resolve/context fallback now surfaces that chain through semantic summaries plus synthesized `DISCOVERS_CONFIG_IN`, `DEPLOYS_FROM`, and `RUNS_ON` relationships; and the Go-owned `trace_deployment_chain` MCP/API path now returns story-first deployment traces with `deployment_overview`, `deployment_sources`, `cloud_resources`, `deployment_facts`, `deployment_fact_summary`, and concrete ArgoCD controller entities in `controller_overview.entities` | deepen canonical backing-resource traversal on top of the now-richer Go trace surface |
| CloudFormation | Go-owned | partial | YAML and JSON detection are native Go; both formats now persist `file_format`, cross-stack import/export buckets, first-class condition definitions, and nested-stack `template_url` metadata; and nested `AWS::CloudFormation::Stack` resources now surface that template URL on the Go entity-context path as a synthesized `DEPLOYS_FROM` relationship | condition evaluation and cross-file nested-stack linking remain partial |
| Kustomize | Go-owned | partial | overlays, resources, base references, and inline patch targets parse in Go; base references stay first-class as a sorted list; typed Kustomize evidence for resources vs Helm vs images materializes in Go; normalized `resource_refs`, `helm_refs`, and `image_refs` now persist on the parser payload; and both the historical patch-link heuristic plus typed deploy-source relationships now survive on the Go-owned entity-context/query path | decide whether any graph-first repository/story/MCP surfacing still matters beyond Python-era parity |
...
- Kotlin: secondary constructors now surface through semantic summaries on normal query/context responses, explicit `this.` receiver calls materialize canonically, primary constructor calls inside function bodies now resolve to class entities, and both obvious local dotted calls, typed local infix calls, plus explicit typed property/local alias chains now infer receiver type strongly enough to materialize canonical edges, but broader receiver/type inference and graph/story promotion are still partial
- PHP: arbitrary receiver-chain and interprocedural object-call inference beyond the currently bounded typed-property and local-alias proof
...
- Rust: impl blocks now persist as `ImplBlock` content entities, are queryable through `code/language-query`, and now expose exact impl ownership through normal entity-context and `code/relationships` fallbacks, but graph implementation edges remain partial
- Elixir: guards, protocols, protocol implementations, and module attributes now appear in semantic summaries on normal query/context responses, the normal Go query path can resolve them through semantic aliases, and `defprotocol` now materializes as a first-class `Protocol` content entity, but the graph still stores those semantics as generic modules/functions/variables
- ArgoCD: deepen canonical backing-resource traversal on top of the now-Go-owned `trace_deployment_chain` MCP/API surfacing for the Application/ApplicationSet evidence chain
- CloudFormation: condition evaluation and cross-file nested-stack linking
- Kustomize: confirm whether anything beyond the parser/relationships/entity-context path is still required for Python-era parity
