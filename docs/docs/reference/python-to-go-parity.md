...
| Kustomize | Go-owned | partial | overlays, resources, base references, and inline patch targets parse in Go; base references stay first-class as a sorted list; typed Kustomize evidence for resources vs Helm vs images materializes in Go; and the historical patch-link heuristic now survives on the Go-owned query path | finish the final graph/query promotion for the typed deploy-source evidence |
...
- Kustomize: final graph/query promotion of the typed deploy-source evidence
