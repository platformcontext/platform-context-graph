...
| --- | --- | --- | --- | --- |
| SQL core parsing | Go-owned | mostly complete | schema objects, migrations, embedded SQL hints, `CREATE OR REPLACE FUNCTION` bodies, legacy `EXECUTE PROCEDURE` trigger wiring, and checked-in `ALTER TABLE ... ADD COLUMN ...` column materialization are natively covered in Go | close the remaining procedural SQL and broader DDL edge cases as needed |
