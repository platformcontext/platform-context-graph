...
- Python: broader graph-first promotion beyond the now-covered `language-query`, `code/search`, `entities/resolve`, `dead-code`, `code/complexity`, and entity-context/story surfaces; parser extraction and current semantic-entity materialization for decorators, async functions, generator functions, metaclass ownership, module docstrings, type annotations, and lambda ownership are already in place
