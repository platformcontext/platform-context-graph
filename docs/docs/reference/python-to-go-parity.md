...
| JavaScript | Go-owned | partial | core JS parsing plus docstring and method-kind metadata extraction/materialization are present, and graph-backed `language-query` plus `code/search` results now enrich matching rows with that metadata | promote docstrings and method-kind metadata into first-class graph/story/context surfaces beyond the query responses |
| TypeScript | Go-owned | partial | core TS parsing plus decorator/generic metadata preservation are present; type aliases are queryable through Go content-backed APIs, and graph-backed `language-query` plus `code/search` results now enrich matching rows with that metadata | promote decorators and generics into first-class normal graph/story/context surfaces |
...
- TypeScript: graph/story/context surfacing for decorators and generics, with graph-backed `language-query` and `code/search` metadata enrichment now in place and type aliases already queryable through Go content-backed APIs
...
- Python: graph/story/context surfacing for decorators and async functions, with graph-backed `language-query` and `code/search` metadata enrichment now in place and type annotations already queryable through Go content-backed APIs
- JavaScript: graph/story/context promotion beyond the now-enriched `language-query` and `code/search` metadata surfaces
