...
Tools for inspecting the status of the deployed ingestor runtimes.
...
| **`get_ingester_status`** | Show detailed status for one ingestor runtime, including retry timing and repo progress counts. | "What is the repository ingestor doing right now?" |
