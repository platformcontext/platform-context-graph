...
| **`find_dead_code`** | Identify derived dead-code candidates, optionally scoped by canonical `repo_id`, with default entrypoint, Go public-API, test, and generated-code exclusions plus optional decorator-owned entry-point exclusions. | "Find dead code in this repo, but ignore `@route`." |
